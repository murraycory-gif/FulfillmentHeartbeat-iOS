...
2. Filter chip **District 03**: hub paints from **`packs/seat/district/03/current.sqlite`**. Market `current.sqlite` is **not** the hub primary. Missing pack **errors** — does not keep old tiles.
...
6. iPad stays cool; dashboard scroll snappy. Stamp **HB-0828.387  1.0 (713)**. Mac: Pages + center only — **no Alerts rail**. Glance tiles: **one health-tinted icon**, no spark bars. District (and OM / Store) section pages: **exactly one** "Store · N stores · tap to expand" — never two stacked.
7. Missing seat object (airplane / unpublished grain) **fails** the hub — does not invent a pack from `current.sqlite`.
8. Command Center: 3 navy heroes + real glance tiles. Mac: pinned Pages + center, **no Alerts column**. iPad land/port: **full-width center**; Pages / Alerts open as drawers only.
...
11. Section scorecards: store table **only** in District / OM / Store — **one** expand/store table on every MetricSection page (Sales included). Company + Region + Division = rollup only.
