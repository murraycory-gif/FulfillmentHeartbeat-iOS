...
| testCompanyWideLostRevenueRegionsAllHaveDollars | A | **A:** both `PulseCaches.build` and `PulseQuery.paint` report 2160 lost-revenue fact stores. `hasMetricFact` / HB-0828.456 count stores with a value, not roster gold 2161. East 612 was not in the fail list. The company headline is the Power BI Total Opportunity market row, `$3,337,325` (HB-0828.369–372, `c0de108`). The `$3,456,041` expectation predates that rule. `testPulseQueryFilterMatchesPowerBI` still expects `$3,456,041` because that path sums store facts and skips the market row. |
...
| testMarketFiveStarTilesMatchCalloutMetrics | A | HB-0828.175 (`0cfdfaf`) set the flag order to Flash, COE, OTT, "Pre Sub OOS%", OTH 5%. The August test expected the older names. The spec list is that order, and the test expects it. |
