# HB-0828.410 / 736 — verify (Mac Share To / Back reachable)

**Tip** `origin/cursor/command-center-8b-3389` · [PR #5](https://github.com/murraycory-gif/FulfillmentHeartbeat-iOS/pull/5)
**Stamp** `HB-0828.410  1.0 (736)` · bundle `com.corymurray.FulfillmentHeartbeat`
**No TestFlight until this PASSes.** Do not delete the app. Cloud Linux cannot `xcodebuild` or talk to a Mac/iPad (no self-hosted worker registered). 410 puts Mac Share Back + To in-content so Catalyst title chrome cannot clip them.
...
| Check | Pass |
|---|---|
| Sidebar stamp | `HB-0828.410  1.0 (736)` |
...
| Automated test | `testArchitecture410MacShareComposeReachable` + 408/406/407 Soft KEEP green |
...
| Mac Share | **New Message** opens at **1100×860**. Navy **Back** capsule is obvious (not a faint title-bar item). **To** is a rounded field you can click; subject + notes sit below it — not clipped / not under the window chrome. +/− resizes. Preview is in the scroll under the fields. Notes go into the mail on Send. Mail still dismisses the sheet first (730 hop). |
...
1. Confirm stamp **HB-0828.410  1.0 (736)**. Pages stays on phone. No phone Back chevron. Tighter CC / section cards.
...
5. **Mac:** no top Pages button; Hide pages collapses the rail; resize the window — callouts and Command Center stay inside the frame. Share → New Message: navy Back, clickable To, notes, +/− size. Send still hops Mail after dismiss.
