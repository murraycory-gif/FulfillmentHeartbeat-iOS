# HB-0828.473 / 787 — verify (PPH count hydrate; pickers under filters + even iPad scorecard columns; live sqlite Sales, not stale facts.json)

**Tip** `cursor/mac-catalyst-category-70dd` off `cursor/mac-share-attach-1006` (`300eb83`)
**Stamp** `HB-0828.473  1.0 (787)` · bundle `com.corymurray.FulfillmentHeartbeat`
...
| Check | Pass |
|---|---|
| Sidebar stamp | `HB-0828.473  1.0 (787)` |
...
| **Picker headcount** | Division = Jewel Osco: Picker tile is shopper headcount with a status, not **0 / No data**. PPH PICKERS: California and South are pack totals (not 0). West is every West market, not Haggen-only 77. East is Jewel + Mid-Atlantic + Shaws, not Jewel-only. |
| **ScoreCard columns** | iPad rows fill the card evenly (Sales, Pick Path, Prep, Dynacap, 5 Star, Labor, Loss, Schedule, PPH, and Picker shopper rows). Phone cards stay put. |
