...
| iPhone | **PhoneCommandCenterHome** (portrait-first): 1-col scroll. Sales / Loss / 5 Star are full-width navy cards (≥124pt). Glance is PhoneScorecardRow list, not a 2-col leftover grid. Content starts below Pages + Filters + banner. **Every section page** (Sales, Loss, Missing Items, 5 Star, Pre-Sub, Pick Path, Prep Not Ready, Dynacap, Schedule Quality, Picker, PPH, Labor) is **PhoneSectionPage** — same scroll shell. Sales week + By Day are OverviewSalesPhoneCard rows, not OverviewSalesBlock in a List. Compact chrome: Pages + Back sit beside the mark, never on top of it. 44pt hits. Markets/Regions: no Unassigned 21. United Week 27 binds live % — never invent. |
