...
final class HeartbeatMathTests: XCTestCase {
    ...
    func testArchitecture428PickPathStoreExpandListsJoinedShoppers() throws {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
    ...
    func testArchitecture397PhoneFilterSwapPaintsCachedSeat() {
        ...
        XCTAssertTrue(PulseLaunch.shouldDelaySectionSQL(seatAlreadyPainted: true))
    ...
    func testArchitecture398PhonePageNavAndFilterSwapAreBothInstant() {
        ...
        XCTAssertEqual(PulseLaunch.maxWarmScorecardHosts(), 2)
        XCTAssertEqual(PulseLaunch.maxWarmScorecardHosts(phone: true), 3)
        XCTAssertEqual(
            PulseLaunch.warmScorecardList(existing: [.labor, .sales], incoming: .pph, cap: 3),
            [.labor, .sales, .pph]
        )
        XCTAssertEqual(
            PulseLaunch.warmScorecardList(existing: [.labor, .sales, .pph], incoming: .dynacap, cap: 3),
            [.sales, .pph, .dynacap]
        )
        ...
        XCTAssertEqual(
            ...
            "seatPaint must not re-arm SQL — that remounts warm phone hosts"
        )
        XCTAssertNotEqual(
            PulseLaunch.sectionSQLTaskToken(section: .sales, filterSummary: "District 03", isActive: true),
    ...
    func testArchitecture427MacCatalystHasAppCategory() throws {
        ...
        if FileManager.default.fileExists(atPath: plistURL.path) {
            ...
            XCTAssertTrue(pbx.contains("CURRENT_PROJECT_VERSION = 771;"))
        }
    ...
    func testArchitecture439PhoneNavSheetParksPackWork() {
        ...
        XCTAssertFalse(PulseLaunch.shouldBindSeatPaintOnHiddenPhoneSection())
        XCTAssertTrue(PulseLaunch.shouldBindSeatPaintOnPhoneSection(isVisible: true))
        XCTAssertFalse(PulseLaunch.shouldBindSeatPaintOnPhoneSection(isVisible: false))
        XCTAssertTrue(PulseLaunch.shouldUseCachedSummariesOnCompactNav())
        XCTAssertTrue(PulseLaunch.shouldDeferHubInvalidationWhileSheetOpen())
        XCTAssertTrue(PulseLaunch.shouldShowBuildStampOnPhonePages())
        XCTAssertFalse(PulseLaunch.shouldCancelSeatSwapWhenSheetOpens())
        XCTAssertFalse(PulseLaunch.shouldPublishHeldSeatPaintAfterSheet())
        XCTAssertFalse(PulseLaunch.shouldParkSectionSQLWhileSheetOpen())
        XCTAssertTrue(PulseLaunch.shouldPublishVisibleSectionAfterLoad(dest: .sales, section: .sales))
        XCTAssertTrue(PulseLaunch.shouldPublishVisibleSectionAfterLoad(dest: .dashboard, section: .sales))
        XCTAssertFalse(PulseLaunch.shouldPublishVisibleSectionAfterLoad(dest: .labor, section: .sales))
        XCTAssertTrue(PulseLaunch.shouldPaintSectionChromeBeforeSQL())
        XCTAssertTrue(PulseLaunch.shouldForceLoadPageGrainsAfterChrome())
        XCTAssertTrue(PulseLaunch.shouldForceLoadDeferredCompanionAfterChrome())
        XCTAssertFalse(PulseLaunch.shouldLoadPickPathPickerOnPageOpen())
        XCTAssertTrue(PulseLaunch.shouldInstallSliceWhenWarehouseHasRows())
        XCTAssertTrue(PulseLaunch.shouldFreezeWarmHostsDuringInteractivePaint())
        XCTAssertTrue(PulseLaunch.shouldCoalesceStampsDuringPageOpen())
        XCTAssertTrue(PulseLaunch.shouldDelaySectionSQL(seatAlreadyPainted: true))
        XCTAssertEqual(
            PulseLaunch.requiredChromeThenFillSections(),
            [.sales, .prepNotReady, .pickPath, .fiveStar, .lostRevenue]
        )
        XCTAssertEqual(PulseLaunch.pageOpenForceLoadSections(.sales), [.sales])
        XCTAssertEqual(PulseLaunch.pageOpenForceLoadSections(.prepNotReady), [.prepNotReady])
        XCTAssertEqual(PulseLaunch.pageOpenForceLoadSections(.fiveStar), [.fiveStar])
        XCTAssertEqual(PulseLaunch.pageOpenForceLoadSections(.lostRevenue), [.lostRevenue])
        XCTAssertEqual(
            PulseLaunch.pageOpenForceLoadSections(.pickPath),
            [.pickPath, .aisleMapper, .pickPathPicker],
            "After chrome, Pick Path must force-load Sequence + shoppers"
        )
        XCTAssertEqual(PulseLaunch.pageOpenForceLoadSections(.preSubOOS), [.preSubOOS, .preSubOOSItem])
        XCTAssertEqual(
            PulseLaunch.warmScorecardList(
                existing: [.labor, .sales, .pph],
                incoming: .dynacap,
                cap: 3,
                freeze: true
            ),
            [.labor, .sales, .pph, .dynacap],
            "Freeze must not evict warm hosts mid page-open"
        )
        XCTAssertEqual(PulseLaunch.maxWarmScorecardHosts(phone: true), 3)
        XCTAssertTrue(
            BuildStamp.label.hasPrefix(BuildStamp.id),
            "Phone Pages must use the same BuildStamp.label as iPad / Mac"
        )
        let snap = CompactNavHealth.snapshot(summaries: [
            SectionSummary(
                section: .sales,
                storeCount: 10,
                headline: 1,
        ...
        XCTAssertEqual(snap.health(for: .sales), .watch)
        XCTAssertEqual(snap.health(for: .labor), .risk)
        XCTAssertEqual(snap.health(for: .dashboard), .risk)
        XCTAssertFalse(PulseLaunch.shouldReloadSectionSQLOnSeatPaintStamp())
        XCTAssertEqual(
            PulseLaunch.sectionSQLTaskToken(
                section: .labor,
                filterSummary: "District 03",
                isActive: true,
                seatPaint: 8
            ),
            PulseLaunch.sectionSQLTaskToken(
        ...
        XCTAssertTrue(
            hub?.contains("final class HubSheetPresenter") == true,
            "Pages / Share flags must live off HubRouter so warm hosts do not rebuild"
        )
        XCTAssertTrue(
            hub?.contains("private struct PhoneSheetHost") == true,
            "Sheet present must not sit on MainHubView.body"
        )
        XCTAssertTrue(hub?.contains("struct HubBuildStamp") == true)
        XCTAssertTrue(hub?.contains("Text(BuildStamp.label)") == true)
        XCTAssertTrue(hub?.contains("shouldShowBuildStampOnPhonePages()") == true)
        XCTAssertTrue(hub?.contains("store.endInteractiveSheet()") == true)
        ...
        XCTAssertTrue(chrome?.contains("store.beginInteractiveSheet()") == true)
        XCTAssertTrue(chrome?.contains("sheets.presentCompactMenu(") == true)
        XCTAssertTrue(hub?.contains("struct CompactNavHealth") == true)
        XCTAssertTrue(hub?.contains("navHealth.health(for:") == true || hub?.contains("sheets.navHealth") == true)
        XCTAssertTrue(hub?.contains("freeze: store.shouldFreezeWarmHosts") == true)
        let page = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
        ...
        XCTAssertTrue(
            page?.contains("store.openSectionPage(section)") == true,
            "Page open must force-load grains after chrome — Soft FAIL blank forever"
        )
    }
    ...
    func testArchitecture440PathExpandDropsPPHMergeAndPrepUsesSnapshots() {
        ...
        XCTAssertFalse(PulseLaunch.shouldLoadPickPathPickerOnPageOpen())
        XCTAssertFalse(PulseLaunch.shouldInventPrepRateOnEmptyStore())
        XCTAssertEqual(PulseLaunch.prepRateText(nil), "—")
        ...
        let scorecardOnly = PulseLaunch.pickPathExpandShoppers(
            scorecard: [scorecard],
            pathRows: [],
            store: "1"
        )
        XCTAssertTrue(
            PulseLaunch.pickPathExpandShowsPlaceholder(scorecardOnly),
            "scorecard-first index must not invent Path expand shoppers"
        )
        let joined = PulseLaunch.pickPathExpandShoppers(
            scorecard: [scorecard],
            pathRows: [path],
            store: "1"
        )
        XCTAssertEqual(joined.count, 1)
        XCTAssertEqual(joined.first?.section, .pickPathPicker)
        XCTAssertEqual(joined.first?.number("compliance_pct") ?? 0, 81.36, accuracy: 0.01)
        XCTAssertEqual(
            PulseLaunch.pickPathPercentAfterLoad(row: joined[0], picker: { _ in path }, loaded: true) ?? 0,
            81.36,
            accuracy: 0.01
        )
        XCTAssertNotEqual(
            PulseLaunch.pickPathExpandDisplay(path: joined.first?.number("compliance_pct"), pph: 76.8, orders: 22).path,
            "—"
        )

        let excelZero = MetricRow(
            section: .prepNotReady,
            division: "Southwest",
            operationsOM: "Nellie Collins",
            storeNumber: "1",
        ...
        XCTAssertEqual(PulseLaunch.prepRateText(0), "0%")
        XCTAssertEqual(PulseLaunch.prepRateText(excelZero.number("pnr_rate_pct")), "0%")
        XCTAssertNotEqual(PulseLaunch.prepRateText(excelZero.number("pnr_rate_pct")), "—")
        let scored = HeartbeatMath.dashboardTableValues(.prepNotReady, rows: [excelZero])
        XCTAssertEqual(scored.values.first, "0%")
        XCTAssertNotEqual(scored.values.first, "—")
        let empty = HeartbeatMath.summarize(.prepNotReady, rows: [], upload: nil)
        XCTAssertEqual(empty.headlineText, "0%")
        XCTAssertEqual(empty.secondary, "No Prep rows this week")
        ...
        XCTAssertTrue(
            page?.contains("private var prepRows: [MetricRow] {\n        let joined = snapshots") == true,
            "Prep pad table must use snapshots / seatRows — not rosterJoined dual ID"
        )
        XCTAssertFalse(
            page?.contains("let joined = store.rosterJoined(for: .prepNotReady)") == true,
            "rosterJoined vs snapshots dual ID is the Prep dash"
        )
        let shoppers = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("FulfillmentHeartbeat/Views/SharedViews.swift"),
        ...
        XCTAssertTrue(
            shoppers?.contains("shouldMergePPHPickersOnPickPathExpand()") == true,
    ...
    func testArchitecture440SeatFirstShellPhonePadOnly() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
        XCTAssertTrue(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertTrue(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertTrue(PulseLaunch.shouldMountSeatHub(mac: false, phone: true, pad: false))
        XCTAssertTrue(PulseLaunch.shouldMountSeatHub(mac: false, phone: false, pad: true))
        XCTAssertFalse(PulseLaunch.shouldMountSeatHub(mac: true, phone: false, pad: false))
        ...
        XCTAssertEqual(
            PulseLaunch.shouldHideShopperListsOnSeatShell(mac: false, filters: company),
            PulseLaunch.shouldUseSeatFirstShell(mac: false)
        )
        ...
        XCTAssertTrue(
            root?.contains("shouldMountSeatHub(") == true,
            "Root must gate SeatHubView behind the P1 flag"
        )
        ...
        XCTAssertTrue(pbx?.contains("CURRENT_PROJECT_VERSION = 771;") == true)
    }

    /// Cory 2026-09-20: first-principles seat shell — banner, title, scoreboard,
    /// child tables, Store pickers. Mac stays Command Center.
    func testArchitecture441FirstPrinciplesSeatShell() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
        XCTAssertTrue(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertTrue(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertFalse(PulseLaunch.shouldUseSeatFirstShell(mac: true))
        XCTAssertTrue(PulseLaunch.shouldKeepMacCommandCenterShell(mac: true))
        XCTAssertFalse(PulseLaunch.shouldMountSeatHubBrandBar())
        XCTAssertTrue(PulseLaunch.shouldUseSeatChromeBanner())
        XCTAssertFalse(PulseLaunch.shouldOpenSectionFromSeatScoreboard())
        XCTAssertFalse(PulseLaunch.shouldShowSectionPagesOnSeatShell())
        XCTAssertFalse(PulseLaunch.shouldShowThisSeatCallout())

        let company = DashboardFilters()
        XCTAssertEqual(PulseLaunch.seatLevelChip(.company), "Company")
        XCTAssertEqual(PulseLaunch.seatGrainCrumb(filters: company), "Company")
        XCTAssertEqual(PulseLaunch.sectionRollupGrains(filters: company), [.region, .division])
        XCTAssertFalse(PulseLaunch.shouldShowStoreTable(filters: company))
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: company))
        ...
        var region = DashboardFilters()
        region.region = "NorCal"
        XCTAssertEqual(PulseLaunch.seatLevelChip(.region), "Region")
        XCTAssertEqual(PulseLaunch.seatGrainCrumb(filters: region), "Company › NorCal")
        XCTAssertEqual(PulseLaunch.sectionRollupGrains(filters: region), [.division])
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: region))

        var division = DashboardFilters()
        division.region = "NorCal"
        division.division = "Jewel Osco"
        XCTAssertEqual(PulseLaunch.sectionRollupGrains(filters: division), [.district])
        XCTAssertTrue(PulseLaunch.shouldShowStoreTable(filters: division))
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: division))
        ...
        XCTAssertEqual(PulseLaunch.sectionRollupGrains(filters: district), [])
        XCTAssertTrue(PulseLaunch.shouldShowStoreTable(filters: district))
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: district))

        var om = DashboardFilters()
        om.om = "Jino Arvin"
        XCTAssertEqual(PulseLaunch.seatLevelChip(.om), "OM")
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: om))

        var store = DashboardFilters()
        store.store = "12"
        XCTAssertTrue(PulseLaunch.shouldShowPickersOnSeatPage(filters: store))
        XCTAssertEqual(PulseLaunch.seatGrainCrumb(filters: store), "Company › Store 12")

        let drilledRegion = PulseLaunch.drillSeatFilters(
            current: company,
            grain: .region,
            rawLabel: "NorCal"
        )
        XCTAssertEqual(drilledRegion.region, "NorCal")
        XCTAssertTrue(drilledRegion.division.isEmpty)
        XCTAssertEqual(PulseLaunch.sectionPageSeat(filters: drilledRegion), .region)

        let drilledMarket = PulseLaunch.drillSeatFilters(
            current: drilledRegion,
            grain: .division,
            rawLabel: "Jewel Osco"
        )
        XCTAssertEqual(drilledMarket.division, "Jewel Osco")
        XCTAssertTrue(drilledMarket.store.isEmpty)
        XCTAssertEqual(PulseLaunch.sectionPageSeat(filters: drilledMarket), .division)

        let drilledStore = PulseLaunch.drillSeatFilters(
            current: drilledMarket,
            grain: .store,
            rawLabel: "12"
        )
        XCTAssertEqual(drilledStore.store, "12")
        XCTAssertEqual(PulseLaunch.sectionPageSeat(filters: drilledStore), .store)

        XCTAssertEqual(PulseLaunch.seatScoreboardColumns(width: 390, pad: false), 2)
        XCTAssertEqual(PulseLaunch.seatScoreboardColumns(width: 1024, pad: true), 3)
        XCTAssertEqual(PulseLaunch.seatScoreboardColumns(width: 1366, pad: true), 4)
        XCTAssertGreaterThan(
            PulseLaunch.seatScoreboardColumns(width: 1366, pad: true),
            PulseLaunch.seatScoreboardColumns(width: 390, pad: false)
        )
        ...
        let shell = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("FulfillmentHeartbeat/Views/SeatHubView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(shell?.contains("struct SeatChromeBanner") == true)
        XCTAssertTrue(shell?.contains("struct SeatScoreTile") == true)
        XCTAssertTrue(shell?.contains("Scoreboard") == true)
        XCTAssertFalse(
            shell?.contains(".hubChrome(") == true,
            "Seat shell must not mount HubBrandBar dual headers"
        )
        XCTAssertTrue(
            shell?.contains("PhoneSectionPage(") == true,
            "Seat page stacks every KPI module for the current who-filter"
        )
        XCTAssertTrue(shell?.contains("embeddedInSeat: true") == true)
        XCTAssertFalse(PulseLaunch.shouldOpenSectionFromSeatScoreboard())
        XCTAssertFalse(PulseLaunch.shouldShowSectionPagesOnSeatShell())
    }

    /// Architecture P1: grain-first ON for iPhone + iPad cable PASS. Mac stays off.
    func testArchitecture442SeatFirstPhoneFlagDefaultOff() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
        XCTAssertTrue(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertTrue(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertFalse(PulseLaunch.shouldKeepSectionCompactNavWhileSeatFlagOff())
        XCTAssertFalse(PulseLaunch.shouldMountEverySeatMetricHostAtOnce())
        XCTAssertTrue(PulseLaunch.shouldDeferSeatPageHeavyUntilAfterChrome())
        XCTAssertFalse(PulseLaunch.shouldLoadFatCompanyPickers())
        XCTAssertTrue(PulseLaunch.shouldKeepMacCommandCenterShell(mac: true))
        XCTAssertFalse(PulseLaunch.shouldShowSectionPagesOnSeatShell())
        ...
        XCTAssertEqual(PulseLaunch.seatPageChromeSections(), [.sales, .lostRevenue, .fiveStar])
        XCTAssertFalse(PulseLaunch.shouldMountEverySeatMetricHostAtOnce())
        ...
        XCTAssertTrue(root?.contains("shouldMountSeatHub(") == true)
        XCTAssertTrue(root?.contains("MainHubView()") == true)
        XCTAssertTrue(root?.contains("SeatHubView()") == true)
        ...
        let launch = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("FulfillmentHeartbeat/Storage/PulseLaunch.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(launch?.contains("static func seatFirstPhoneEnabled() -> Bool { true }") == true)
        XCTAssertTrue(launch?.contains("static func shouldEnableSeatFirstOnIPad() -> Bool { true }") == true)
        XCTAssertFalse(
            launch?.contains("static func seatFirstPhoneEnabled() -> Bool { false }") == true
        )
        XCTAssertFalse(
            launch?.contains("static func shouldEnableSeatFirstOnIPad() -> Bool { false }") == true
        )
    }

    /// Cory design bar: KPI tiles + instant seat switcher. Phone + iPad ON.
    func testArchitecture443SeatDesignBar() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
        XCTAssertTrue(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertTrue(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertTrue(PulseLaunch.shouldKeepMacCommandCenterShell(mac: true))
        ...
        XCTAssertTrue(PulseLaunch.shouldMountSeatHub(mac: false, phone: false, pad: true))
        XCTAssertFalse(PulseLaunch.shouldMountSeatHub(mac: true, phone: false, pad: false))
        XCTAssertTrue(PulseLaunch.shouldUseSeatDesignBar())
        XCTAssertTrue(PulseLaunch.shouldUseInstantSeatSwitcher())
        XCTAssertFalse(PulseLaunch.shouldPreferFilterSheetOverSeatSwitcher())
        XCTAssertFalse(PulseLaunch.shouldShowVanityChartsOnSeatPage())
        XCTAssertFalse(PulseLaunch.shouldLetChromeFightSeatNumbers())
        XCTAssertFalse(PulseLaunch.shouldShowSeatAssistInChrome())
        XCTAssertTrue(PulseLaunch.shouldUseSeatKPIStatusDelta())
        XCTAssertTrue(PulseLaunch.shouldShowAllSeatMetricsInOneScroll())
        XCTAssertFalse(PulseLaunch.shouldLoadFatCompanyPickers())
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: DashboardFilters()))

        var division = DashboardFilters()
        division.region = "NorCal"
        division.division = "Jewel Osco"
        let crumbs = PulseLaunch.seatSwitcherCrumbs(filters: division)
        XCTAssertEqual(crumbs.map(\.seat), [.company, .region, .division])
        XCTAssertEqual(crumbs.map(\.title), ["Company", "NorCal", "Jewel Osco"])
        let popped = PulseLaunch.popSeatFilters(current: division, to: .region)
        XCTAssertEqual(popped.region, "NorCal")
        XCTAssertTrue(popped.division.isEmpty)
        XCTAssertEqual(PulseLaunch.sectionPageSeat(filters: popped), .region)
        XCTAssertEqual(PulseLaunch.seatSwitcherSiblingFocus(filters: DashboardFilters()), .region)
        XCTAssertEqual(PulseLaunch.seatSwitcherSiblingFocus(filters: division), .division)

        let switched = PulseLaunch.applySeatSwitcherChoice(
            current: division,
            focus: .division,
            value: "Safeway"
        )
        XCTAssertEqual(switched.region, "NorCal")
        XCTAssertEqual(switched.division, "Safeway")
        XCTAssertTrue(switched.store.isEmpty)

        var storeOnly = DashboardFilters()
        storeOnly.store = "12"
        XCTAssertFalse(PulseLaunch.shouldShowSeatSwitcherSiblings(filters: storeOnly))
        XCTAssertTrue(PulseLaunch.shouldShowPickersOnSeatPage(filters: storeOnly))
        var districtStore = storeOnly
        districtStore.district = "03"
        XCTAssertTrue(PulseLaunch.shouldShowSeatSwitcherSiblings(filters: districtStore))
        ...
        XCTAssertEqual(PulseLaunch.seatKPIStatusText(sales), "Watch")
        XCTAssertEqual(PulseLaunch.seatKPIDeltaText(sales), "+16.2% YoY")
        XCTAssertEqual(PulseLaunch.seatKPIActionLine(sales), "Watch · +16.2% YoY")
        ...
        XCTAssertTrue(PulseLaunch.seatKPIActionLine(loss).contains("At risk"))
        XCTAssertTrue(PulseLaunch.seatKPIActionLine(loss).contains("3.40%"))
        ...
        XCTAssertTrue(shell?.contains("Instant seat switcher") == true)
        XCTAssertTrue(shell?.contains("seatKPIActionLine") == true)
        XCTAssertTrue(shell?.contains("KPIs") == true)
        XCTAssertFalse(shell?.contains("HealthBadge(") == true)
        XCTAssertFalse(shell?.contains("sparkles") == true)
        XCTAssertFalse(shell?.contains("Chart") == true)
        XCTAssertTrue(shell?.contains("PhoneSectionPage(") == true)
        XCTAssertTrue(shell?.contains("embeddedInSeat: true") == true)
        XCTAssertFalse(
            shell?.contains("ForEach(FilterFocus.allCases)") == true,
            "Soft FAIL FilterSheet pills as the primary seat switch"
        )
    }

    /// App Design memo v3: one-glance KPIs + stacked modules + Clear/crumb. Flags ON.
    func testArchitecture444AppDesignMemoV3SeatShell() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
        XCTAssertTrue(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertTrue(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertTrue(PulseLaunch.shouldMountSeatHub(mac: false, phone: true, pad: false))
        ...
        XCTAssertFalse(PulseLaunch.shouldMountSeatHub(mac: true, phone: false, pad: false))
        XCTAssertTrue(PulseLaunch.shouldKeepMacCommandCenterShell(mac: true))
        XCTAssertTrue(PulseLaunch.shouldPaintAllSeatKPIsAtAGlance())
        XCTAssertEqual(PulseLaunch.seatGlanceKPISections(), PulseLaunch.seatPageMetricSections())
        XCTAssertGreaterThanOrEqual(PulseLaunch.seatGlanceKPISections().count, 10)
        XCTAssertTrue(PulseLaunch.shouldShowSeatActionableStrip())
        XCTAssertTrue(PulseLaunch.shouldShowSeatBannerClearAndCrumbOnly())
        XCTAssertFalse(PulseLaunch.shouldShowShareInSeatBanner())
        XCTAssertFalse(PulseLaunch.shouldShowSiblingChipsInSeatBanner())
        XCTAssertFalse(PulseLaunch.shouldReplaceSeatKPINumbersWithCharts())
        XCTAssertFalse(PulseLaunch.shouldShowVanityChartsOnSeatPage())
        XCTAssertFalse(PulseLaunch.shouldLetChromeFightSeatNumbers())
        XCTAssertFalse(PulseLaunch.shouldMountEverySeatMetricHostAtOnce())
        XCTAssertFalse(PulseLaunch.shouldLoadFatCompanyPickers())
        ...
        XCTAssertEqual(PulseLaunch.seatKPIDeltaText(watch), "-2.1% YoY")
        let strip = PulseLaunch.seatActionableStripLine([good, watch, risk])
        XCTAssertTrue(strip.contains("Loss"))
        XCTAssertTrue(strip.contains("At risk"))
        XCTAssertTrue(strip.contains("Sales"))
        XCTAssertTrue(strip.contains("Watch"))
        XCTAssertFalse(strip.contains("5 Star"))
        XCTAssertTrue(PulseLaunch.seatActionableStripLine([good]).isEmpty)

        var division = DashboardFilters()
        division.region = "NorCal"
        division.division = "Jewel Osco"
        let crumbs = PulseLaunch.seatSwitcherCrumbs(filters: division)
        XCTAssertEqual(crumbs.first?.seat, .company)
        XCTAssertEqual(PulseLaunch.popSeatFilters(current: division, to: .company).isActive, false)
        XCTAssertEqual(
            PulseLaunch.sectionRollupGrains(filters: DashboardFilters()),
            [.region, .division]
        )
        ...
        XCTAssertTrue(shell?.contains("Button(\"Clear\")") == true)
        XCTAssertTrue(shell?.contains("seatSwitcherCrumbs") == true)
        XCTAssertTrue(shell?.contains("seatActionableStripLine") == true)
        XCTAssertTrue(shell?.contains("shouldPaintAllSeatKPIsAtAGlance") == true)
        XCTAssertTrue(shell?.contains("Executive KPI scoreboard") == true)
        XCTAssertTrue(shell?.contains("seatTitle") == true)
        XCTAssertTrue(shell?.contains("metricStack") == true)
        XCTAssertTrue(shell?.contains("seatPageMetricSections") == true)
        XCTAssertTrue(shell?.contains("PhoneSectionPage(") == true)
        XCTAssertTrue(shell?.contains("embeddedInSeat: true") == true)
        XCTAssertFalse(shell?.contains("siblingSwitcher") == true)
        XCTAssertFalse(shell?.contains("HealthBadge(") == true)
        XCTAssertFalse(shell?.contains("Chart") == true)
        XCTAssertFalse(shell?.contains("ForEach(FilterFocus.allCases)") == true)
        XCTAssertFalse(
            shell?.contains("shouldShowShareInSeatBanner") == true
                && PulseLaunch.shouldShowShareInSeatBanner()
        )
    }

    /// HB-0828.445: grain-first seat shell ON for iPhone + iPad cable PASS.
    /// Who-filter only. Every KPI module stacked. No section Pages. Mac untouched.
    func testArchitecture445SeatFirstPhoneAndIPadEnabled() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.445")
        XCTAssertTrue(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertTrue(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertTrue(PulseLaunch.shouldMountSeatHub(mac: false, phone: true, pad: false))
        ...
        XCTAssertEqual(PulseLaunch.sectionRollupGrains(filters: region), [.division])
        XCTAssertFalse(PulseLaunch.shouldShowPickersOnSeatPage(filters: region))
        XCTAssertEqual(PulseLaunch.seatPageMetricSections(), PulseLaunch.seatGlanceKPISections())

        let stacked = PulseLaunch.seatPageMetricSections()
        XCTAssertTrue(stacked.contains(.sales))
        XCTAssertTrue(stacked.contains(.lostRevenue))
        XCTAssertTrue(stacked.contains(.missingItems))
        XCTAssertTrue(stacked.contains(.fiveStar))
        XCTAssertTrue(stacked.contains(.preSubOOS))
        XCTAssertTrue(stacked.contains(.pickPath))
        XCTAssertTrue(stacked.contains(.prepNotReady))
        XCTAssertTrue(stacked.contains(.dynacap))
        XCTAssertTrue(stacked.contains(.scheduleQuality))
        XCTAssertTrue(stacked.contains(.pph))
        XCTAssertTrue(stacked.contains(.labor))
        XCTAssertTrue(stacked.contains(.pickerScorecard))
        ...
        XCTAssertTrue(shell?.contains("PhoneSectionPage(section: section, embeddedInSeat: true)") == true)
        XCTAssertTrue(shell?.contains(".id(store.filters.summary)") == true)
        XCTAssertFalse(shell?.contains(".hubChrome(") == true)
        ...
        XCTAssertTrue(brand?.contains("if !HubLayout.usesSeatFirstShell") == true)
        XCTAssertTrue(brand?.contains("title: \"Pages\"") == true)
    }
}
