...
final class HeartbeatMathTests: XCTestCase {
    ...
    func testDynacapDistrictFileParsesAndJoinsStores() {
        ...
        XCTAssertEqual(Set(parsed.map { $0.textPayload["district"] ?? "" }), Set(["J1", "J2", "39"]))
        XCTAssertEqual(parsed.first { ($0.textPayload["district"] ?? "") == "J1" }?.payload["dynacap_rate"] ?? 0, 74.07, accuracy: 0.02)
        let roster = [
    ...
    func testPickPathShopperPathNeverNilWhenPathRowExists_allDivisions() {
        let divisions = MarketRegion.officialDivisions
        XCTAssertFalse(divisions.isEmpty)
        var rows: [MetricRow] = []
        let scoreOnlyId = "SCOREONLY"
        for (index, division) in divisions.enumerated() {
            let district = "D\(index)"
            let om = "OM \(division)"
            for slot in 0..<2 {
                let store = String(8000 + index * 2 + slot)
                let shopper = "P\(index)S\(slot)"
                let pathPct = 70.0 + Double(index) + Double(slot) / 10.0
                let pathPPH = 40.0 + Double(index)
                let pathOrders = 100.0 + Double(slot)
                rows.append(rosterRow(division: division, om: om, store: store, district: district))
                rows.append(scorecardRow(
                    division: division, om: om, store: store, district: district,
                    shopper: shopper, pph: 1, orders: 1
                ))
                rows.append(pathPickerRow(
                    division: division, om: om, store: store, district: district,
                    shopper: shopper, path: pathPct, pph: pathPPH, orders: pathOrders
                ))
                rows.append(storePathRow(division: division, om: om, store: store, district: district))
            }
        }
        let emptyStore = "8999"
        rows.append(rosterRow(division: divisions[0], om: "OM \(divisions[0])", store: emptyStore, district: "D0"))
        rows.append(scorecardRow(
            division: divisions[0], om: "OM \(divisions[0])", store: emptyStore, district: "D0",
            shopper: scoreOnlyId, pph: 9, orders: 9
        ))

        var covered = Set<String>()
        for division in divisions {
            covered.insert(division)
            var filters = DashboardFilters()
            filters.division = division
            let caches = PulseCaches.build(rows: rows, filters: filters, uploads: [])
            let stores = caches.cachedStores.map(\.0)
            XCTAssertGreaterThanOrEqual(stores.count, 2, division)
            for store in stores {
                let pathRows = PulseLaunch.pickPathPickers(store: store, rows: caches.pickPathPickersByStore)
                let scorecard = caches.pphPickersByStore[HeartbeatMath.canonicalStore(store)] ?? []
                let storePath = (caches.filteredLatest[.pickPath] ?? []).first {
                    HeartbeatMath.sameStore($0.storeNumber, store)
                }
                ...
                let scoreOnlyKey = HeartbeatMath.canonicalShopper(scoreOnlyId)
                XCTAssertFalse(lines.contains { $0.id == scoreOnlyKey || $0.name == scoreOnlyId })
                if pathRows.isEmpty {
                    XCTAssertTrue(
                        PulseLaunch.pickPathShopperLinesAreEmptyNotice(lines),
                        "\(division) \(store) must show No Path Picker rows"
                    )
                    XCTAssertEqual(lines.first?.name, PulseLaunch.noPathPickerRowsTitle)
                    continue
                }
                XCTAssertFalse(pathRows.contains { $0.section != .pickPathPicker })
                let listed = lines.filter { $0.id != PulseLaunch.noPathPickerRowsID }
                XCTAssertFalse(listed.isEmpty, "\(division) \(store)")
                for line in listed {
                    XCTAssertNotNil(line.path, "\(division) \(store) \(line.name) Path %")
                    XCTAssertNotNil(line.pph, "\(division) \(store) \(line.name) PPH")
                    XCTAssertNotNil(line.orders, "\(division) \(store) \(line.name) Orders")
                    let path = pathRows.first { HeartbeatMath.shopperAliases($0).contains(line.id) }
                    XCTAssertNotNil(path, "\(division) \(store) \(line.id)")
                    XCTAssertEqual(line.path ?? -1, path?.number("compliance_pct") ?? -2, accuracy: 0.01)
                    XCTAssertEqual(line.pph ?? -1, path?.number("pph") ?? -2, accuracy: 0.01)
                    XCTAssertEqual(line.orders ?? -1, path?.number("orders") ?? -2, accuracy: 0.01)
                    XCTAssertFalse(line.path == nil && (line.pph != nil || line.orders != nil))
                }
            }
        }
    ...
    func testPickPathShopperLaunchStoreReadsArgument() {
        ...
        XCTAssertNil(PulseLaunch.pickPathShopperLaunchStore(arguments: ["App"]))
        let empty = PulseLaunch.pickPathShopperLines(pathRows: [], scorecardRows: [], storePath: nil)
        XCTAssertEqual(empty.first?.name, PulseLaunch.noPathPickerRowsTitle)
    ...
    private func rosterRow(division: String, om: String, store: String, district: String) -> MetricRow {
        MetricRow(
            ...
            operationsOM: om,
            storeNumber: store,
            textPayload: ["district": district, "roster": "1"]
        )
    }

    private func scorecardRow(
        division: String, om: String, store: String, district: String,
        shopper: String, pph: Double, orders: Double
    ) -> MetricRow {
        MetricRow(
            section: .pickerScorecard,
            division: division,
            operationsOM: om,
            storeNumber: store,
            payload: ["pph": pph, "orders": orders],
            textPayload: ["shopper_id": shopper, "shopper_name": shopper, "district": district]
        )
    }

    private func pathPickerRow(
        division: String, om: String, store: String, district: String,
        shopper: String, path: Double, pph: Double, orders: Double
    ) -> MetricRow {
        MetricRow(
            section: .pickPathPicker,
            division: division,
            operationsOM: om,
            storeNumber: store,
            payload: ["compliance_pct": path, "pph": pph, "orders": orders],
            textPayload: ["shopper_id": shopper, "shopper_name": shopper, "district": district]
        )
    }

    private func storePathRow(division: String, om: String, store: String, district: String) -> MetricRow {
        MetricRow(
            section: .pickPath,
            division: division,
            operationsOM: om,
            ...
            textPayload: [
                "district": district,
                AisleMapperMath.mapperKey: "2026-08-20",
                AisleMapperMath.sequenceKey: "2026-09-16",
            ]
        )
    }

    func testArchitecture428PickPathStoreExpandListsJoinedShoppers() throws {
        XCTAssertEqual(BuildStamp.id, "HB-0828.478")
        ...
        XCTAssertTrue(
            PulseLaunch.shouldPeekStoreSeatForPickPathExpand(activeGrain: .company, storePackUsable: true)
        )
        XCTAssertTrue(
            PulseLaunch.shouldPeekStoreSeatForPickPathExpand(activeGrain: .district, storePackUsable: true)
        )
        XCTAssertFalse(
            PulseLaunch.shouldPeekStoreSeatForPickPathExpand(activeGrain: .store, storePackUsable: true)
        )

        let scorecard = MetricRow(
            section: .pickerScorecard,
            division: "Southwest",
            operationsOM: "Nellie Collins",
            storeNumber: "2",
            payload: ["pph": 76.8, "orders": 6620],
            textPayload: ["shopper_id": "RPAL114", "shopper_name": "RPAL114", "district": "97"]
        )
        let path = MetricRow(
            section: .pickPathPicker,
            division: "",
            operationsOM: "",
            storeNumber: "",
            payload: ["compliance_pct": 84.78, "pph": 76.8, "orders": 22],
            textPayload: ["shopper_id": "RPAL114", "shopper_name": "RPAL114"]
        )
        let other = MetricRow(
            section: .pickerScorecard,
            division: "Jewel Osco",
        ...
        let roster: [String: HeartbeatMath.StoreIdentity] = [
            "2": HeartbeatMath.StoreIdentity(
                division: "Southwest", district: "97", om: "Nellie Collins", name: "2"
            ),
            "3503": HeartbeatMath.StoreIdentity(
                division: "Jewel Osco", district: "J2", om: "Shelly Selof", name: "3503"
            ),
        ]
        let company = PulseSeatPack.scopeRows(
            [scorecard, path, other],
            roster: roster,
            key: .company
        )
        XCTAssertFalse(
            company.contains { PulseSeatPack.shopperSections().contains($0.section) },
            "company thin pack must not write shopper tape"
        )
        let storeRows = PulseSeatPack.scopeRows(
            [scorecard, path, other],
            roster: roster,
            key: PulseSeatPack.Key(grain: .store, id: "2")
        )
        XCTAssertTrue(storeRows.contains { $0.section == .pickerScorecard && $0.storeNumber == "2" })
        let stamped = storeRows.filter { $0.section == .pickPathPicker }
        ...
        let shoppers = PulseLaunch.pickPathPickers(store: "0002", rows: index.rows)
        XCTAssertFalse(shoppers.isEmpty, "store alias 0002 must hit store 2")
        let pathShopper = shoppers.first { $0.section == .pickPathPicker }
        XCTAssertEqual(pathShopper?.number("compliance_pct") ?? 0, 84.78, accuracy: 0.01)
        XCTAssertEqual(HeartbeatFormat.pct(pathShopper?.number("compliance_pct")), "84.78%")
        XCTAssertNotEqual(HeartbeatFormat.pct(pathShopper?.number("compliance_pct")), "0.85%")

        var filters = DashboardFilters()
        filters.store = "2"
        let sliced = PulseLaunch.slicePickPathPickers(
            pathRows: [path],
            scorecard: [scorecard, other],
            allowed: ["2"]
        )
        XCTAssertEqual(sliced.count, 1)
        XCTAssertEqual(sliced.first?.shopperId, "RPAL114")

        let caches = PulseCaches.build(rows: [scorecard, path, other], filters: filters,
            uploads: []
        )
        XCTAssertEqual(caches.filteredLatest[.pickPathPicker]?.count, 1)
        XCTAssertFalse(PulseLaunch.pickPathPickers(store: "2", rows: caches.pickPathPickersByStore).isEmpty)

        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("path-expand-428-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        let dest = tmp.appendingPathComponent("current.sqlite")
        _ = try PulseSeatPack.writeCooked(
            rows: [scorecard, path, other],
            uploads: [],
            key: PulseSeatPack.Key(grain: .store, id: "2"),
            roster: roster,
            to: dest
        )
        let packed = PulseSQLite.readStores(
            from: dest,
            sections: [.pickerScorecard, .pickPathPicker],
            stores: ["2"]
        )
        XCTAssertTrue(
            packed.contains { $0.section == .pickPathPicker && $0.storeNumber == "2" },
            "cooked store pack must return pick_path_picker on readStores"
        )
    }
    ...
    func testArchitecture429PickPathSequenceDatesAndPrepFromThinPack() throws {
        ...
        XCTAssertTrue(PulseQuery.isStoreFact(prep))
        XCTAssertTrue(PulseQuery.isStoreFact(mapper), "aisle mapper dates-only rows must survive slice")
        XCTAssertNil(AisleMapperMath.sequenceISO(path))

        let joined = HeartbeatMath.applyAisleMapper([path], from: [mapper])
        XCTAssertEqual(AisleMapperMath.mapperISO(joined[0]), "2026-08-20")
        XCTAssertEqual(AisleMapperMath.sequenceISO(joined[0]), "2026-09-16")
        XCTAssertEqual(HeartbeatFormat.shortDate(AisleMapperMath.sequenceISO(joined[0])), "9/16/26")
        ...
        XCTAssertEqual(AisleMapperMath.sequenceISO(bySection[.pickPath]![0]), "2026-09-16")
        XCTAssertEqual(bySection[.prepNotReady]?.first?.number("pnr_rate_pct") ?? 0, 3.27, accuracy: 0.01)
        ...
        XCTAssertFalse(
            ...
            "bake must not invent shopper tape"
        )
        ...
        XCTAssertTrue(company.contains { $0.section == .aisleMapper })
        XCTAssertTrue(company.contains { $0.section == .prepNotReady })
        XCTAssertFalse(company.contains { PulseSeatPack.shopperSections().contains($0.section) })

        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("path-dates-429-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        let dest = tmp.appendingPathComponent("current.sqlite")
        _ = try PulseSeatPack.writeCooked(
            rows: [path, mapper, prep],
            uploads: [],
            key: PulseSeatPack.Key(grain: .store, id: "2"),
            roster: roster,
            to: dest
    ...
    func testArchitecture436PickPathExpandLoadsPickerAndPrepUsesStoreNotStoreHash() {
        ...
        XCTAssertFalse(PulseLaunch.shouldShowPickerLoadingOnPickPathExpand(dest: .dashboard))
        XCTAssertFalse(PulseLaunch.shouldLoadSection(visible: .pickPath, section: .pickPathPicker))
        XCTAssertFalse(PulseLaunch.shouldInventPrepRateOnEmptyStore())
        XCTAssertEqual(PulseLaunch.shopperEmptyDetail(loading: true), "Loading shoppers…")
        XCTAssertEqual(PulseLaunch.shopperEmptyDetail(loading: false), "Shoppers fill from the Heartbeat pack after ready.")
        ...
        let scorecard = MetricRow(
            ...
            operationsOM: "Nellie Collins",
            storeNumber: "1",
            payload: ["pph": 76.8, "orders": 22],
            textPayload: ["shopper_id": "AVELJ03", "shopper_name": "AVELJ03", "district": "97"]
        )
        let path = MetricRow(
            section: .pickPathPicker,
            division: "Southwest",
            operationsOM: "Nellie Collins",
            storeNumber: "1",
            payload: ["compliance_pct": 81.36, "pph": 76.8, "orders": 22],
            textPayload: ["shopper_id": "AVELJ03", "shopper_name": "AVELJ03", "district": "97"]
        )
        XCTAssertNil(
            PulseLaunch.pickPathPercentAfterLoad(row: scorecard, picker: { _ in nil }, loaded: false),
            "Path % must wait until pick_path_picker load"
        )
        XCTAssertNil(
            PulseLaunch.pickPathPercentAfterLoad(row: scorecard, picker: { _ in path }, loaded: false)
        )
        XCTAssertEqual(
            PulseLaunch.pickPathPercentAfterLoad(row: path, picker: { _ in path }, loaded: true) ?? 0,
            81.36,
            accuracy: 0.01
        )
        XCTAssertFalse(PulseLaunch.pickPathPercentReady([scorecard]))
        XCTAssertTrue(PulseLaunch.pickPathPercentReady([path]))

        var extras: [MetricRow] = []
        extras.reserveCapacity(13)
        for index in 2...14 {
            extras.append(
                MetricRow(
                    section: .pickPathPicker,
                    ...
                    operationsOM: "Nellie Collins",
                    storeNumber: "1",
                    payload: ["compliance_pct": 80 + Double(index) / 100, "orders": Double(index)],
                    textPayload: ["shopper_id": "SHOP\(index)", "shopper_name": "SHOP\(index)"]
                )
            )
        }
        let shoppers = PulseLaunch.pickPathExpandShoppers(
            scorecard: [scorecard],
            pathRows: [path] + extras,
            store: "1"
        )
        XCTAssertEqual(shoppers.filter { $0.section == .pickPathPicker }.count, 14)
        let avel = shoppers.first { $0.shopperId == "AVELJ03" }
        XCTAssertEqual(avel?.number("compliance_pct") ?? 0, 81.36, accuracy: 0.01)
        XCTAssertEqual(
            PulseLaunch.pickPathExpandDisplay(
                path: avel?.number("compliance_pct"),
                pph: avel?.number("pph"),
                orders: avel?.number("orders")
            ).path,
            "81.36%"
        )

        XCTAssertEqual(
            WorkbookParser.preferredPrepStoreColumnIndex(["Store #", "Store", "Net Prep Not Ready Hours %"]),
            1
        )
        XCTAssertEqual(
            WorkbookParser.preferredPrepStoreColumnIndex(["DIVISION", "Store", "Prep Not Ready Hours %", "Store #"]),
            1
        )
        XCTAssertTrue(WorkbookParser.isStoreHashHeader("Store #"))
        XCTAssertTrue(WorkbookParser.isExactStoreHeader("STORE"))
        ...
        Jewel Osco,J1,Shelly Selof,1,2219,0.07455
        """
        let parsed = WorkbookParser.parseCSV(csv).map { $0.asRow(section: .prepNotReady) }
        XCTAssertEqual(Set(parsed.map(\.storeNumber)), Set(["2", "3427", "2219"]))
        XCTAssertFalse(parsed.contains { $0.storeNumber == "1" }, "Store # = 1 must not cook as store 1")
        XCTAssertEqual(parsed.first { $0.storeNumber == "2" }?.number("pnr_rate_pct") ?? 0, 3.27, accuracy: 0.02)
        ...
        let card = caches.cachedSummaries.first { $0.section == .prepNotReady }
        XCTAssertEqual(card?.headline, 0)
        XCTAssertEqual(card?.headlineText, "0%")
        XCTAssertEqual(card?.secondary, "No Prep rows this week")
        XCTAssertNotEqual(card?.headlineText, "—")
        XCTAssertTrue(PulseLaunch.isPrepEmptyChrome(card!))
        XCTAssertEqual(CommandCenterLayout.compactValue(card!), "0%")

        let empty = HeartbeatMath.summarize(.prepNotReady, rows: [], upload: nil)
        XCTAssertEqual(empty.headlineText, "0%")
        XCTAssertEqual(empty.secondary, "No Prep rows this week")
    ...
    func testArchitecture382CommandCenterFillsViewportLikePulse() {
        ...
        XCTAssertEqual(CommandCenterLayout.phoneHomeStackSpacing(), 5)
        XCTAssertEqual(CommandCenterLayout.phoneHomeHorizontalPadding(), 12)
        XCTAssertEqual(CommandCenterLayout.phoneScorecardChipMinHeight(), 32)
    ...
    func testArchitecture384SeatSwapAndSectionOpenPlane() {
        ...
        XCTAssertFalse(PulseLaunch.shouldLoadSection(visible: .dashboard, section: .pickPathPicker, pushed: .pickPath))
        XCTAssertNotEqual(
    ...
    func testArchitecture420SeatPlaneTinyLRUNoFullCaches() {
        ...
        XCTAssertFalse(PulseLaunch.shouldCacheGrainTablesInSeatRowPlane())
        XCTAssertFalse(PulseLaunch.shouldSnapshotUnfilteredPulseWhenRowPlaneCached())
        XCTAssertFalse(PulseLaunch.shouldBuildFullPulseCachesOnFilterSwap())
        XCTAssertTrue(PulseLaunch.shouldPaintCachedSeatOnFilterTap())
        XCTAssertFalse(PulseLaunch.shouldReinstallSeatPackWhenRowPlanePainted())
        XCTAssertTrue(PulseLaunch.shouldRewriteSeatRowPlaneWithChrome())
        XCTAssertFalse(PulseLaunch.shouldDeferSeatInstallWhenRowPlaneMissing())
        XCTAssertTrue(PulseLaunch.shouldCoalesceSeatPaintStamp())
        XCTAssertTrue(PulseLaunch.shouldSkipRedundantSeatChromeApply())
        XCTAssertFalse(PulseLaunch.shouldLockPickerDashboardOnFilterSwap())
        XCTAssertFalse(PulseLaunch.shouldInstallSeatExpandTablesOnFilterSwap())
        XCTAssertFalse(PulseLaunch.shouldUseHeavySeatCachesOnFilterSwap())
        XCTAssertFalse(PulseLaunch.shouldReloadSectionSQLOnSeatPaintStamp())
    ...
    func testArchitecture421IdleAfterOpenNoHotLoop() {
        ...
        XCTAssertFalse(PulseLaunch.shouldIngestCloudWorkbookOnForeground())
        XCTAssertFalse(PulseLaunch.shouldCookPublishedSeatPlaneInApp())
        XCTAssertFalse(PulseLaunch.shouldFillPickerAfterCompanyReady())
        XCTAssertFalse(PulseLaunch.shouldStreamPickerOnDashboard())
        XCTAssertFalse(PulseLaunch.shouldToggleMacScrollIndicatorsFromFit())
        XCTAssertTrue(PulseLaunch.shouldRewriteSeatRowPlaneWithChrome())
        XCTAssertTrue(PulseLaunch.shouldPaintCachedSeatOnFilterTap())
        XCTAssertFalse(PulseLaunch.shouldReinstallSeatPackWhenRowPlanePainted())
        XCTAssertFalse(PulseLaunch.shouldBuildFullPulseCachesOnFilterSwap())
    ...
    func testArchitecture422BoxMapsRewriteSameTurnAsChrome() {
        ...
        XCTAssertFalse(PulseLaunch.shouldStampSeatPaintBeforeBoxMapsRewrite())
        XCTAssertTrue(PulseLaunch.shouldPublishSeatPaintAfterBoxMapsRewrite())
        XCTAssertTrue(PulseLaunch.shouldRewriteSeatRowPlaneWithChrome())
        XCTAssertTrue(PulseLaunch.shouldPaintCachedSeatOnFilterTap())
        XCTAssertFalse(PulseLaunch.shouldReinstallSeatPackWhenRowPlanePainted())
        XCTAssertFalse(PulseLaunch.shouldDeferSeatInstallWhenRowPlaneMissing())
    ...
    func testArchitecture425MacShareAttachesReportViaShareSheet() {
        ...
        XCTAssertTrue(PulseLaunch.shouldShareReportFile())
        XCTAssertTrue(PulseLaunch.shouldShowShareAttachmentChip())
        XCTAssertTrue(PulseLaunch.shouldUseMailComposeAsFallback(canSendMail: true))
        XCTAssertFalse(PulseLaunch.shouldUseMailComposeAsFallback(canSendMail: false))
        XCTAssertFalse(PulseLaunch.shouldAllowShareSend(to: "ops@company.com", htmlReady: true, fileReady: false))
        XCTAssertTrue(PulseLaunch.shouldAllowShareSend(to: "ops@company.com", htmlReady: true, fileReady: true))
        XCTAssertFalse(PulseLaunch.shouldAllowShareSend(to: "", htmlReady: true, fileReady: true))
        XCTAssertEqual(
            PulseLaunch.shareAttachmentChipTitle(files: [], htmlFile: nil),
            "Preparing attachment…"
        )
        let pdf = URL(fileURLWithPath: "/tmp/Fulfillment-Heartbeat.pdf")
        XCTAssertEqual(
            PulseLaunch.shareAttachmentChipTitle(files: [pdf], htmlFile: nil),
            "Fulfillment-Heartbeat.pdf"
        )
        XCTAssertEqual(PulseLaunch.shareAttachmentMimeType(for: pdf), "application/pdf")
        let pngs = [
            URL(fileURLWithPath: "/tmp/heartbeat-page-1.png"),
            URL(fileURLWithPath: "/tmp/heartbeat-page-2.png"),
        ]
        XCTAssertEqual(PulseLaunch.shareAttachmentChipTitle(files: pngs, htmlFile: nil), "2 page images")
        XCTAssertEqual(PulseLaunch.shareAttachmentMimeType(for: pngs[0]), "image/png")
        ...
        XCTAssertEqual(PulseMail.shareAttachmentFiles(packet), [html])
        let pdfURL = FileManager.default.temporaryDirectory.appendingPathComponent("hb-425.pdf")
        try? Data(repeating: 0x25, count: 120).write(to: pdfURL)
        defer { try? FileManager.default.removeItem(at: pdfURL) }
        XCTAssertGreaterThan(PulseLaunch.fileBytes(at: pdfURL), 80)
        let attached = packet.withAttachments([pdfURL])
        XCTAssertEqual(PulseMail.shareAttachmentFiles(attached), [pdfURL])
        let body = PulseMail.mailComposeHTMLBody(from: attached)
        XCTAssertTrue(body.contains("<p>Recap</p>"))
        XCTAssertFalse(body == attached.brief)
        XCTAssertEqual(
            PulseLaunch.shareSendHandoff(toFilled: true, canSendMail: true, mac: false),
            .mailCompose
        )
        XCTAssertEqual(
            PulseLaunch.shareSendHandoff(toFilled: true, canSendMail: true, mac: true),
            .mailCompose
        )
        XCTAssertEqual(
            PulseLaunch.shareSendHandoff(toFilled: true, canSendMail: false, mac: true),
            .richClipboardMailto
        )
        XCTAssertEqual(
            PulseLaunch.shareSendHandoff(toFilled: true, canSendMail: false, mac: false),
            .richClipboardMailto
        )
        let store = DashboardFilters(region: "", division: "", district: "", om: "", store: "3407")
        XCTAssertEqual(
            store.summary,
            "All regions · All divisions · All districts · All OMs · 3407"
        )
        XCTAssertEqual(
            PulseLaunch.shareAttachmentChipTitle(files: attached.attachmentFiles, htmlFile: html),
    ...
    func testArchitecture426FirstPaintIsCurrentPackGold() {
        ...
        XCTAssertTrue(PulseLaunch.shouldPullCloudPackOnForeground())
        XCTAssertFalse(
            PulseLaunch.shouldRevealDashboardBeforeCurrentPack(
                cloudCheckComplete: false,
                fetchFailed: false,
                localUsable: true
            )
        )
        ...
        XCTAssertTrue(
            ...
            "Offline keeps the local seat after the fetch fails"
        )
        XCTAssertFalse(
            PulseLaunch.shouldRevealDashboardBeforeCurrentPack(
                cloudCheckComplete: false,
        ...
        XCTAssertTrue(
            PulseLaunch.staleCompanySeatRequiresCloudSync(
                ...
                localRowsLoaded: 400,
                remoteUpdated: "2026-09-14T15:00:00Z",
                knownUpdated: "2026-09-07T12:00:00Z",
                localWrittenAt: "2026-09-07T12:00:00Z"
            )
        )
        XCTAssertFalse(PulseLaunch.shouldPreferSystemShareSheetWithFile())
        XCTAssertFalse(PulseLaunch.shouldBounceFilledToToSystemShareSheet())
    ...
    /// Soft FAIL HB 752: Mac Catalyst TestFlight upload rejected ASC 90242
    ...
    func testArchitecture427MacCatalystHasAppCategory() throws {
        ...
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let plistURL = root.appendingPathComponent("FulfillmentHeartbeat/Info.plist")
        var sawCategory = false
        if FileManager.default.fileExists(atPath: plistURL.path) {
            let data = try Data(contentsOf: plistURL)
            let plist = try PropertyListSerialization.propertyList(from: data, format: nil)
            guard let dict = plist as? [String: Any] else {
                return XCTFail("Info.plist is not a dictionary")
            }
            XCTAssertEqual(dict["LSApplicationCategoryType"] as? String, "public.app-category.business")
            sawCategory = true

            let pbx = try String(
                contentsOf: root.appendingPathComponent("FulfillmentHeartbeat.xcodeproj/project.pbxproj"),
                encoding: .utf8
            )
            XCTAssertTrue(
        ...
        XCTAssertTrue(sawCategory, "LSApplicationCategoryType must be in Info.plist or the app bundle")

        // 752 Soft KEEP: first-paint current pack gold + Share / UIActivityViewController
        XCTAssertTrue(PulseLaunch.shouldBlockFirstPaintUntilCurrentPack())
        XCTAssertFalse(PulseLaunch.shouldPaintLocalSeatBeforeCloudFreshnessCheck())
        XCTAssertTrue(PulseLaunch.shouldShowShortLoadUntilCurrentPack())
        XCTAssertFalse(PulseLaunch.shouldPreferSystemShareSheetWithFile())
    ...
    func testPostReadyWorkStaysOffSplashAndCoolsTheHub() {
        ...
        XCTAssertFalse(
            PulseLaunch.shouldLoadPublishedFacts(lostStores: 40, salesStores: 40),
            "A thin or missing sqlite stays empty. Do not paint Sep 12 facts.json."
        )
    ...
    func testArchitecture470PPHIndexSurvivesPackChromeAndDivisionSeat() {
        ...
        XCTAssertTrue(emptyBuckets.hydrateCounts)
        XCTAssertFalse(emptyBuckets.loadFullScorecard)
        let shortCounts = PulseLaunch.pphOpenPlan(
            section: .pph, filtersActive: false, countedStores: 2, countedShoppers: 77, packShoppers: 24_500
        )
        XCTAssertTrue(shortCounts.hydrateCounts)
        XCTAssertFalse(shortCounts.loadFullScorecard)
        let complete = PulseLaunch.pphOpenPlan(
            section: .pph, filtersActive: false, countedStores: 2_161, countedShoppers: 24_500, packShoppers: 24_500
        )
        XCTAssertFalse(complete.hydrateCounts)
        XCTAssertFalse(complete.loadSeatShoppers)
        XCTAssertFalse(complete.loadFullScorecard)
        let seat = PulseLaunch.pphOpenPlan(
        ...
        let pack = PulseLaunch.pickerHeadcountIndex(storeCounts: ["1": 5, "1490": 4])
        let prefix = PulseLaunch.pickerHeadcountIndex(storeCounts: ["1": 2])
        let merged = PulseLaunch.mergedPPHCounts(existing: pack, incoming: prefix)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1", counts: merged), 5)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1490", counts: merged), 4)
        XCTAssertFalse(PulseLaunch.shouldKeepPPHShopperGroup(
            existingCount: 0, incomingCount: 2, knownHeadcount: 5
        ))
        XCTAssertTrue(PulseLaunch.shouldKeepPPHShopperGroup(
            existingCount: 0, incomingCount: 5, knownHeadcount: 5
        ))
        let indexed = PulseLaunch.pphPickerIndex([
            MetricRow(
                section: .pickerScorecard,
        ...
        XCTAssertEqual(indexed.shoppers, 1)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1", counts: indexed.counts), 1)
        for division in MarketRegion.officialDivisions {
            let row = MetricRow(
                section: .pickerScorecard,
                division: division,
                operationsOM: "A",
                storeNumber: "10",
                payload: ["pph": 70, "orders": 2],
                textPayload: ["shopper_id": "S", "shopper_name": "S"]
            )
            let card = HeartbeatMath.summarize(.pickerScorecard, rows: [row], upload: nil)
            XCTAssertEqual(card.headline ?? 0, 1, accuracy: 0.01, division)
            XCTAssertNotEqual(card.health, .none, division)
        }
        let none = HeartbeatMath.summarize(.pickerScorecard, rows: [], upload: nil)
        XCTAssertEqual(none.headline ?? 0, 0, accuracy: 0.01)
        XCTAssertEqual(none.health, .none)
    }
    ...
    func testPickerHeadcountIgnoresDivisionIndexPrefix() {
        ...
        let pack = PulseLaunch.pickerHeadcountIndex(storeCounts: [
            "3427": 77,
            "1": 3_268,
            "1490": 3_310,
            "2500": 3_301,
        ...
        let counts = PulseLaunch.pickerHeadcountsForRollup(pack: pack, streamedPrefix: prefix)
        XCTAssertEqual(PulseLaunch.pphPickerTotal(storeNumbers: ["3427"], counts: counts), 77)
        XCTAssertEqual(PulseLaunch.pphPickerTotal(storeNumbers: ["1"], counts: counts), 3_268)
        XCTAssertEqual(PulseLaunch.pphPickerTotal(storeNumbers: ["1490"], counts: counts), 3_310)
        XCTAssertEqual(PulseLaunch.pphPickerTotal(storeNumbers: ["2500"], counts: counts), 3_301)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "304", counts: counts), 4)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "0304", counts: counts), 4)
        XCTAssertNotEqual(counts, prefix)

        XCTAssertTrue(PulseLaunch.shouldLoadPickerShoppersForActiveFilter(
            filtersActive: true,
            loadedFilterKey: "",
            filterKey: "Jewel Osco",
            shopperRows: 4_000
        ))
        XCTAssertFalse(PulseLaunch.shouldLoadPickerShoppersForActiveFilter(
            filtersActive: true,
            loadedFilterKey: "Jewel Osco",
            filterKey: "Jewel Osco",
            shopperRows: 3_268
        ))
        XCTAssertFalse(PulseLaunch.shouldLoadPickerShoppersForActiveFilter(
        ...
        XCTAssertTrue(PulseLaunch.shouldKeepSeatPickerRows(
            filtersActive: true,
            loadKey: "Jewel Osco",
            filterKey: "Jewel Osco",
            seatShoppers: 3_268
        ))
        XCTAssertFalse(PulseLaunch.shouldKeepSeatPickerRows(
            filtersActive: true,
            loadKey: "",
            filterKey: "Jewel Osco",
            seatShoppers: 4_000
        ...
        let card = HeartbeatMath.summarize(.pickerScorecard, rows: jewel, upload: nil)
        XCTAssertEqual(card.headline ?? 0, 2, accuracy: 0.01)
        XCTAssertNotEqual(card.health, .none)
        let empty = HeartbeatMath.summarize(.pickerScorecard, rows: [], upload: nil)
        XCTAssertEqual(empty.headline ?? 0, 0, accuracy: 0.01)
        XCTAssertEqual(empty.health, .none)

        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("hb-picker-headcount-\(UUID().uuidString).sqlite")
        defer { try? FileManager.default.removeItem(at: url) }
        func shopper(_ id: String, store: String, division: String, pph: Double?) -> MetricRow {
            MetricRow(
                section: .pickerScorecard,
                division: division,
                operationsOM: "A",
                storeNumber: store,
                payload: pph.map { ["pph": $0, "orders": 1] } ?? ["orders": 1],
                textPayload: ["shopper_id": id, "shopper_name": id]
            )
        }
        let rows = [
            shopper("H1", store: "3427", division: "Haggen", pph: 80),
            shopper("J1", store: "1", division: "Jewel Osco", pph: 60),
            shopper("J2", store: "1", division: "Jewel Osco", pph: nil),
            shopper("N1", store: "1490", division: "NorCal", pph: 71),
            shopper("S1", store: "2500", division: "SoCal", pph: 75),
        ]
        XCTAssertNoThrow(try PulseSQLite.write(rows: rows, uploads: [], seeded: true, to: url))
        let sql = PulseLaunch.pickerHeadcountIndex(storeCounts: PulseSQLite.pickerHeadcounts(from: url))
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "3427", counts: sql), 1)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1", counts: sql), 2)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1490", counts: sql), 1)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "2500", counts: sql), 1)
        XCTAssertEqual(
            PulseLaunch.pphPickerTotal(storeNumbers: ["1490", "2500"], counts: sql),
            2
        )
        let seeded = PulseSQLite.readSection(from: url, section: .pickerScorecard, limit: 1, offset: 0)
        XCTAssertEqual(seeded.count, 1)
        XCTAssertGreaterThan(PulseSQLite.sectionCount(from: url, section: .pickerScorecard), seeded.count)
    }
    ...
    func testPPHOpenUsesPackGroupByCensusNotFullScorecard() {
        ...
        defer { try? FileManager.default.removeItem(at: url) }
        let bands: [(store: String, division: String, count: Int)] = [
            ("1", "Jewel Osco", 3_233),
            ("200", "Shaws", 2_000),
            ("300", "Mid-Atlantic", 2_688),
            ("1490", "NorCal", 3_300),
            ("2500", "SoCal", 3_311),
            ("400", "United", 680),
            ("500", "Southern", 2_000),
            ...
            ("700", "Seattle", 2_000),
            ("800", "Portland", 1_800),
            ("900", "Mountain West", 1_739),
        ]
        var rows: [MetricRow] = []
        rows.reserveCapacity(24_535)
        for band in bands {
            for index in 0..<band.count {
                rows.append(MetricRow(
                    ...
                    operationsOM: "A",
                    storeNumber: band.store,
                    payload: ["orders": 1],
                    textPayload: ["shopper_id": "\(band.store)-\(index)", "shopper_name": "\(band.store)-\(index)"]
                ))
            }
        }
        XCTAssertNoThrow(try PulseSQLite.write(rows: rows, uploads: [], seeded: true, to: url))
        let counts = PulseLaunch.pickerHeadcountIndex(storeCounts: PulseSQLite.pickerHeadcounts(from: url))
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1", counts: counts), 3_233)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "400", counts: counts), 680)
        func regionTotal(_ region: MarketRegion) -> Int {
            bands.filter { region.contains($0.division) }.reduce(0) { partial, band in
                partial + PulseLaunch.pphPickerCount(store: band.store, counts: counts)
            }
        }
        XCTAssertEqual(regionTotal(.east), 7_921)
        XCTAssertEqual(regionTotal(.california), 6_611)
        XCTAssertEqual(regionTotal(.south), 4_387)
        XCTAssertEqual(regionTotal(.west), 5_616)
        let prefix = PulseLaunch.pickerHeadcountIndex(storeCounts: ["1": 4])
        let merged = PulseLaunch.mergedPPHCounts(existing: counts, incoming: prefix)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "1", counts: merged), 3_233)
        XCTAssertEqual(PulseLaunch.pphPickerCount(store: "400", counts: merged), 680)
        let expanded = PulseSQLite.readStores(from: url, sections: [.pickerScorecard], stores: ["0001"])
        XCTAssertEqual(expanded.count, 3_233)
        XCTAssertTrue(expanded.allSatisfy { HeartbeatMath.canonicalStore($0.storeNumber) == "1" })
        let counted = PulseLaunch.countedPPHShoppers(counts)
        let plan = PulseLaunch.pphOpenPlan(
            section: .pph,
            filtersActive: false,
            countedStores: counted.stores,
            countedShoppers: counted.shoppers,
            packShoppers: counted.shoppers
        )
        XCTAssertFalse(plan.hydrateCounts)
        XCTAssertFalse(plan.loadFullScorecard)
        XCTAssertEqual(counted.shoppers, 24_535)
    }
    ...
    /// HB-0828.474: iPad Regions/Markets rows fill the card. Leftover width is an
    /// even gutter once columns hit a snug cap, so Pick Path ORDERS is not a canyon
    /// and Sales / Prep do not leave a dead zone after Status. Phone cards stay put.
    func testScorecardColumnsFillIPadWithoutAMetricCanyon() throws {
        let pad: CGFloat = 1_288
        let sales = ScorecardColumns.fit(
            available: pad,
            columns: ScorecardColumns.row(metrics: ScorecardColumns.salesMetrics, showCount: true),
            phone: false
        )
        let path = ScorecardColumns.fit(
            available: pad,
            columns: ScorecardColumns.row(metrics: ScorecardColumns.pickPathMetrics, showCount: true),
            phone: false
        )
        let prep = ScorecardColumns.fit(
            available: pad,
            columns: ScorecardColumns.row(metrics: ScorecardColumns.prepMetrics, showCount: true),
            phone: false
        )
        XCTAssertEqual(sales.widths.count, 11)
        XCTAssertEqual(path.widths.count, 6)
        XCTAssertEqual(prep.widths.count, 6)
        XCTAssertEqual(
            ScorecardColumns.row(metrics: ScorecardColumns.prepMetrics, showCount: true).count,
            6
        )
        XCTAssertEqual(
            ScorecardColumns.row(metrics: ScorecardColumns.dynacapMetrics, showCount: true).count,
            7
        )
        XCTAssertEqual(
            ScorecardColumns.row(metrics: ScorecardColumns.fiveStarMetrics, showCount: true).count,
            9
        )
        if !HubLayout.isMac {
            XCTAssertEqual(sales.rowWidth, pad, accuracy: 0.75)
            XCTAssertEqual(path.rowWidth, pad, accuracy: 0.75)
            XCTAssertGreaterThanOrEqual(sales.gutter, 8)
            XCTAssertGreaterThan(path.gutter, sales.gutter + 16)
            let pathMetrics = path.widths.dropFirst(2).dropLast()
            XCTAssertLessThan(pathMetrics.max() ?? 0, pad * 0.16)
            XCTAssertLessThan(path.widths.last ?? 0, 150)
            let salesMetrics = sales.widths.dropFirst(2).dropLast()
            XCTAssertGreaterThan(salesMetrics.max() ?? 0, salesMetrics.min() ?? 0)
            XCTAssertEqual(prep.rowWidth, pad, accuracy: 0.75)
            let prepMetrics = prep.widths.dropFirst(2).dropLast()
            XCTAssertLessThan(prepMetrics.max() ?? 0, 160)
            let prepEnd = prep.widths.reduce(0, +) + prep.gutter * CGFloat(prep.widths.count - 1)
            XCTAssertEqual(prepEnd, pad, accuracy: 0.75)
            let picker = ScorecardColumns.fit(
                available: pad,
                columns: ScorecardColumns.row(metrics: ScorecardColumns.pickerMetrics, showCount: false),
                phone: false
            )
            XCTAssertEqual(picker.widths.count, 9)
            XCTAssertEqual(picker.rowWidth, pad, accuracy: 0.75)
            let pickerMetrics = picker.widths.dropFirst().dropLast()
            XCTAssertLessThan(pickerMetrics.max() ?? 0, 160)
        }
        let phone = ScorecardColumns.fit(
            available: 390,
            columns: ScorecardColumns.row(metrics: ScorecardColumns.salesMetrics, showCount: true),
            phone: true
        )
        XCTAssertGreaterThan(phone.rowWidth, 700)
        XCTAssertGreaterThanOrEqual(phone.widths.min() ?? 0, 52)
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let salesSource = try String(contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SalesViews.swift"), encoding: .utf8)
        XCTAssertTrue(salesSource.contains("usesPhoneScorecards"))
        XCTAssertTrue(salesSource.contains("OverviewSalesPhoneCard"))
        XCTAssertTrue(salesSource.contains("ScorecardColumns.salesMetrics"))
        XCTAssertFalse(salesSource.contains("enum SalesCols"))
        let shared = try String(contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SharedViews.swift"), encoding: .utf8)
        XCTAssertTrue(shared.contains("ScorecardColumns.pickPathMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.prepMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.dynacapMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.fiveStarMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.laborMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.lostMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.scheduleMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.pphMetrics"))
        XCTAssertTrue(shared.contains("ScorecardColumns.pickerMetrics"))
        XCTAssertTrue(shared.contains("PickerPhoneCard"))
        XCTAssertTrue(shared.contains("usesPhoneScorecards"))
        XCTAssertTrue(shared.contains("HubScrollSpan"))
        XCTAssertTrue(shared.contains("PrepMetricHeader"))
    }
    ...
    func testShareDashboardMatchesOnScreenCalloutsAndOpensWithoutHTML() {
        ...
        XCTAssertTrue(packet.html.contains("<td class=\"pill\" bgcolor=\"#D97706\""), packet.html)
    ...
    func testArchitecture446PhoneHeroesUseLaborScorecardChrome() {
        ...
        XCTAssertFalse(PulseLaunch.shouldPublishPickerSeatFirstPaint())
        XCTAssertFalse(PulseLaunch.shouldUsePagingScroll())
        XCTAssertEqual(CommandCenterLayout.phoneScorecardCorner(), 14)
        XCTAssertEqual(CommandCenterLayout.phoneScorecardAccentWidth(), 5)

        let healthy = SectionSummary(
            section: .sales,
            storeCount: 2161,
            headline: 16_860_000,
            headlineLabel: "Sales",
        ...
        XCTAssertEqual(CommandCenterLayout.phoneScorecardSubtitle(healthy), "2161 stores")
        XCTAssertEqual(CommandCenterLayout.phoneScorecardChips(healthy).first?.label, "Result")
        XCTAssertEqual(CommandCenterLayout.phoneScorecardChips(healthy).first?.value, "$16.86M")
        XCTAssertEqual(CommandCenterLayout.combinedHealth([healthy, risk]), .risk)

        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let command = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/CommandCenterView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(
            command?.contains("struct PhoneCommandHeroCard") == true
        )
        XCTAssertTrue(
            command?.contains("chips: CommandCenterLayout.phoneScorecardChips(painted)") == true,
            "Phone KPI heroes must reuse PhoneScorecardRow chips"
        )
        XCTAssertTrue(
            command?.contains(".background(AppTheme.blue, in: RoundedRectangle(cornerRadius: 14, style: .continuous))") == true,
            "Mac Command Center leftover-fill navy tiles stay"
        )
        ...
        XCTAssertTrue(
            brand?.contains("overviewBannerCopy(") == true,
            "Phone page banner must use filter-aware Overview copy"
        )
        XCTAssertFalse(
            brand?.contains("accessory: compactBannerAccessory") == true,
            "Phone page banner must not keep navy HubBanner accessory layout"
        )
    }
    ...
    func testArchitecture447CompactPageBannerIsNotScorecardRow() {
        ...
        XCTAssertTrue(PulseLaunch.shouldLeavePadMacCommandChromeUnchanged())
        XCTAssertFalse(PulseLaunch.shouldShowThisSeatCallout())
        XCTAssertEqual(CommandCenterLayout.phonePageBannerMaxHeight(), 88)
        XCTAssertLessThan(CommandCenterLayout.phonePageBannerMaxHeight(), CommandCenterLayout.phoneHeroMinHeight())
        ...
        guard let brand,
              let start = brand.range(of: "private var compactPageBanner"),
              let end = brand.range(of: "private var compactBannerHealth")
        else {
            return XCTFail("compactPageBanner must stay on HubBrandBar")
        }
        let banner = String(brand[start.lowerBound..<end.lowerBound])
        XCTAssertTrue(banner.contains("PhoneCompactPageBanner"), banner)
        XCTAssertFalse(banner.contains("PhoneScorecardRow"), banner)
        XCTAssertTrue(banner.contains("overviewBannerCopy("), banner)
        ...
        XCTAssertTrue(
            command?.contains("chips: CommandCenterLayout.phoneScorecardChips(painted)") == true,
            "Sales / Loss / 5 Star must keep Labor RESULT chips"
        )
        let dash = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/DashboardView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(dash?.contains("struct PhoneCompactPageBanner") == true)
        XCTAssertTrue(dash?.contains(".fixedSize(horizontal: false, vertical: true)") == true)
        XCTAssertTrue(command?.contains("func phonePageBannerMaxHeight()") == true)
    }
    ...
    func testArchitecture448NavyOverviewBannerKeepsWhiteHeroes() {
        ...
        XCTAssertEqual(
            CommandCenterLayout.overviewBannerCopy(filters: DashboardFilters(), weekWindow: nil),
            "Total Company | Current Week"
        )
        XCTAssertEqual(CommandCenterLayout.overviewWeekLabel(nil), "Current Week")
        XCTAssertEqual(CommandCenterLayout.overviewWeekLabel(""), "Current Week")
        XCTAssertEqual(CommandCenterLayout.overviewWeekLabel("  "), "Current Week")
        XCTAssertEqual(CommandCenterLayout.overviewWeekLabel("Week 29"), "Week 29")

        var district = DashboardFilters()
        district.district = "J2"
        XCTAssertEqual(
            CommandCenterLayout.overviewBannerCopy(filters: district, weekWindow: nil),
            "J2 District | Current Week"
        )

        var region = DashboardFilters()
        region.region = "Intermountain"
        XCTAssertEqual(
            CommandCenterLayout.overviewSeatLabel(region),
        ...
        division.division = "Jewel Osco"
        XCTAssertEqual(
            CommandCenterLayout.overviewSeatLabel(division),
            "Jewel Osco Division"
        )

        var om = DashboardFilters()
        om.om = "Luke Lomas"
        XCTAssertEqual(CommandCenterLayout.overviewSeatLabel(om), "Luke Lomas OM")

        var store = DashboardFilters()
        store.store = "3427"
        XCTAssertEqual(CommandCenterLayout.overviewSeatLabel(store), "Store 3427")
        store.district = "J2"
        XCTAssertEqual(
            CommandCenterLayout.overviewSeatLabel(store),
            "Store 3427",
            "finest grain wins — store over district"
        )
        ...
        guard let dash,
              let bannerStart = dash.range(of: "struct PhoneCompactPageBanner"),
              let bannerEnd = dash.range(of: "struct PhoneScorecardRow")
        else {
            return XCTFail("PhoneCompactPageBanner must stay next to PhoneScorecardRow")
        }
        let banner = String(dash[bannerStart.lowerBound..<bannerEnd.lowerBound])
        XCTAssertTrue(banner.contains(".background(AppTheme.blue"), "banner must be brand navy")
        XCTAssertFalse(banner.contains("background(Color.white"), "navy overview banner must not be white Labor chrome")
        XCTAssertFalse(
            banner.contains("overlay(alignment: .leading)"),
            "navy overview banner must not keep white Labor accent strip"
        )
        let command = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/CommandCenterView.swift"),
            encoding: .utf8
    ...
    func testArchitecture449PhoneDropsAtAGlanceCaption() {
        ...
        let command = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("FulfillmentHeartbeat/Views/CommandCenterView.swift"),
            encoding: .utf8
        )
        guard let command,
              let start = command.range(of: "struct PhoneCommandCenterHome"),
              let end = command.range(of: "struct PhoneCommandHeroCard")
        else {
            return XCTFail("PhoneCommandCenterHome must stay above PhoneCommandHeroCard")
        }
        let phoneHome = String(command[start.lowerBound..<end.lowerBound])
        XCTAssertFalse(
            phoneHome.contains("AT-A-GLANCE"),
            "Phone home must not show AT-A-GLANCE · ALL SECTIONS"
        )
        XCTAssertTrue(
            command.contains("private var glanceHeader"),
            "Mac leftover-fill glance header stays"
        )
        XCTAssertTrue(
            command.contains("chips: CommandCenterLayout.phoneScorecardChips(painted)"),
            "Sales / Loss / 5 Star must keep Labor RESULT chips"
        )
    }
    ...
    func testArchitecture450CompactPagesFiltersShareToolbar() {
        ...
        let shared = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("FulfillmentHeartbeat/Views/SharedViews.swift"),
            encoding: .utf8
        )
        guard let shared,
              let toolbarStart = shared.range(of: "struct HubCompactActionToolbar"),
              let toolbarEnd = shared.range(of: "struct FilterBar"),
              let barStart = shared.range(of: "private var compactBar"),
              let barEnd = shared.range(of: "private var regularShowsHeaderPages")
        else {
            return XCTFail("HubCompactActionToolbar must stay next to FilterBar")
        }
        let toolbar = String(shared[toolbarStart.lowerBound..<toolbarEnd.lowerBound])
        XCTAssertTrue(toolbar.contains("title: \"Pages\""), toolbar)
        XCTAssertTrue(toolbar.contains("FilterBar()"), toolbar)
        XCTAssertTrue(toolbar.contains("HubChromePill("), toolbar)
        let brand = String(shared[barStart.lowerBound..<barEnd.lowerBound])
        XCTAssertFalse(brand.contains("title: \"Pages\""), "Pages must not stay on the logo row")
        XCTAssertTrue(shared.contains("HubCompactActionToolbar(showsFilters: showsFilters)"))
        XCTAssertTrue(
            shared.contains("hubChrome(showBack:"),
            "One shared HubBrandBar chrome — not per-page headers"
        )
    }
    ...
    func testArchitecture451CompactNavCloseIsFinishedPill() {
        ...
        let root = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let hub = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/MainHubView.swift"),
            encoding: .utf8
        )
        guard let hub,
              let sheetStart = hub.range(of: "struct CompactNavSheet"),
              let sheetEnd = hub.range(of: "private func navRow")
        else {
            return XCTFail("CompactNavSheet must stay the shared Pages sheet")
        }
        let sheet = String(hub[sheetStart.lowerBound..<sheetEnd.lowerBound])
        XCTAssertTrue(sheet.contains("HubSheetCloseControl"), sheet)
        XCTAssertFalse(sheet.contains("cancellationAction"), sheet)
        XCTAssertFalse(sheet.contains("Button(\"Close\")"), sheet)
        ...
        guard let shared,
              let closeStart = shared.range(of: "struct HubSheetCloseControl"),
              let closeEnd = shared.range(of: "struct HubNavControl")
        else {
            return XCTFail("HubSheetCloseControl must stay a shared chrome atom")
        }
        let close = String(shared[closeStart.lowerBound..<closeEnd.lowerBound])
        XCTAssertTrue(close.contains(".buttonStyle(.plain)"), close)
        XCTAssertTrue(close.contains("AppTheme.blue"), close)
        XCTAssertFalse(close.contains("phoneHitTarget"), close)
    }
    ...
    func testArchitecture452CompactNavCloseLivesInSheetContent() {
        ...
        let hub = try? String(
            contentsOf: URL(fileURLWithPath: #filePath)
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("FulfillmentHeartbeat/MainHubView.swift"),
            encoding: .utf8
        )
        guard let hub,
              let sheetStart = hub.range(of: "struct CompactNavSheet"),
        ...
        let sheet = String(hub[sheetStart.lowerBound..<sheetEnd.lowerBound])
        XCTAssertTrue(sheet.contains("HubSheetCloseControl"), sheet)
        XCTAssertTrue(sheet.contains("safeAreaInset(edge: .top"), sheet)
        XCTAssertFalse(sheet.contains(".toolbar"), sheet)
        XCTAssertFalse(sheet.contains("topBarLeading"), sheet)
        XCTAssertFalse(sheet.contains("cancellationAction"), sheet)
        XCTAssertTrue(sheet.contains("Text(\"SECTIONS\")"), sheet)
    }
    ...
    func testArchitecture453CompactFilterSheetFlow() {
        ...
        guard let shared,
              let sheetStart = shared.range(of: "struct FilterSheet"),
              let sheetEnd = shared.range(of: "struct FilterColumn")
        else {
            return XCTFail("FilterSheet must stay the shared filter sheet")
        }
        let sheet = String(shared[sheetStart.lowerBound..<sheetEnd.lowerBound])
        XCTAssertTrue(sheet.contains("filterGrainChips"), sheet)
        XCTAssertTrue(sheet.contains("filterActionBar"), sheet)
        XCTAssertTrue(sheet.contains("safeAreaInset(edge: .bottom"), sheet)
        XCTAssertTrue(sheet.contains("HubSheetCloseControl(title: \"Cancel\")"), sheet)
        XCTAssertTrue(sheet.contains("HubSheetCloseControl(title: \"Save\""), sheet)
        XCTAssertFalse(sheet.contains("Search or tap rows"), sheet)
        XCTAssertFalse(
            sheet.contains("LazyVGrid"),
            "Phone filter grains are one chip row, not a 2-col grid that orphans Store"
        )
    }
    ...
    func testArchitecture454CompactBannerPageOverFilterSeat() {
        ...
        XCTAssertEqual(CommandCenterLayout.overviewPageTitle(.dashboard), "Dashboard")
        XCTAssertEqual(CommandCenterLayout.overviewPageTitle(.sales), "Sales")
        XCTAssertEqual(CommandCenterLayout.overviewPageTitle(.labor), "Labor")
        XCTAssertEqual(CommandCenterLayout.overviewPageTitle(.lostRevenue), "Loss Revenue")

        var division = DashboardFilters()
        division.division = "Jewel Osco"
        XCTAssertEqual(
            CommandCenterLayout.overviewBannerCopy(filters: division, weekWindow: nil),
            "Jewel Osco Division | Current Week"
        )
        XCTAssertNotEqual(
            CommandCenterLayout.overviewPageTitle(.dashboard),
            CommandCenterLayout.overviewSeatLabel(division),
            "Soft FAIL filter grain on line 1"
        )
        ...
        let mounted = String(brand[start.lowerBound..<end.lowerBound])
        XCTAssertTrue(mounted.contains("title: CommandCenterLayout.overviewPageTitle("), mounted)
        XCTAssertTrue(mounted.contains("subtitle: CommandCenterLayout.overviewBannerCopy("), mounted)
        XCTAssertFalse(
            mounted.contains("title: CommandCenterLayout.overviewSeatLabel("),
            "Soft FAIL filter seat as the primary banner title"
        )
    ...
    func testArchitecture455CompanyThisWeekRollupAboveRegions() {
        ...
        var division = DashboardFilters()
        division.division = "Jewel Osco"
        XCTAssertFalse(PulseLaunch.shouldShowCompanyThisWeekRollup(filters: region))
        XCTAssertFalse(PulseLaunch.shouldShowCompanyThisWeekRollup(filters: division))
        XCTAssertEqual(PulseLaunch.sectionRollupGrains(filters: company), [.region, .division])

        let sections: [MetricSection] = [
            .lostRevenue, .missingItems, .fiveStar, .preSubOOS, .pickPath,
            .prepNotReady, .dynacap, .scheduleQuality, .pickerScorecard, .pph, .labor,
        ]
        for section in sections {
            XCTAssertTrue(
                PulseLaunch.shouldShowMetricCompanyThisWeekRollup(section: section, filters: company),
                section.rawValue
            )
            XCTAssertFalse(
                PulseLaunch.shouldShowMetricCompanyThisWeekRollup(section: section, filters: region),
                section.rawValue
            )
        }
        ...
        guard let page,
              let heavyStart = page.range(of: "private var heavyBlocks"),
              let heavyEnd = page.range(of: "private var seatMetricCard")
        else {
            return XCTFail("PhoneSectionPage heavyBlocks must stay the phone section stack")
        }
        let heavy = String(page[heavyStart.lowerBound..<heavyEnd.lowerBound])
        XCTAssertTrue(heavy.contains("PhoneCompanyThisWeekBlock(section: section)"), heavy)
        XCTAssertTrue(heavy.contains("grainBlock(grain)"), heavy)
        if let week = heavy.range(of: "PhoneCompanyThisWeekBlock"),
           let grains = heavy.range(of: "ForEach(PulseLaunch.sectionRollupGrains") {
            XCTAssertLessThan(week.lowerBound, grains.lowerBound, "This Week must sit above Regions")
        } else {
            XCTFail("This Week must mount before the Regions ForEach")
        }

        guard let blockStart = page.range(of: "struct PhoneCompanyThisWeekBlock"),
              let blockEnd = page.range(of: "struct PhoneSectionHeading")
        else {
            return XCTFail("PhoneCompanyThisWeekBlock must share region-card chrome")
        }
        let block = String(page[blockStart.lowerBound..<blockEnd.lowerBound])
        XCTAssertTrue(block.contains("PhoneScorecardRow("), block)
        XCTAssertTrue(block.contains("dashboardTableValues("), block)
        XCTAssertTrue(block.contains("\"This Week\""), block)
        XCTAssertTrue(block.contains("overviewSeatLabel("), block)
        XCTAssertTrue(block.contains("OverviewSalesPhoneCard"), block)
        XCTAssertFalse(block.contains("PhoneCommandHeroCard"), block)
    }
    ...
    func testArchitecture456MetricPageStoreCountIsFactStores() {
        ...
        func row(_ store: String, pph: Double?) -> MetricRow {
            MetricRow(
                section: .pph,
                division: "Jewel Osco",
                operationsOM: "OM",
        ...
        XCTAssertEqual(HeartbeatMath.metricStoreCount(.pph, rows: rows), 4)
        XCTAssertEqual(
            HeartbeatMath.metricStoreCount(.pph, rows: rows),
            HeartbeatMath.metricStoreCount(.pph, rows: Array(rows.prefix(4))),
            "hero / This Week / Regions share one fact-store filter"
        )

        var rosterPinned = SectionSummary(
            section: .pph,
            storeCount: 2161,
            headline: 74.2,
            headlineLabel: "PPH",
        ...
        rosterPinned = PulseLaunch.companyCommandCenterCard(
            rosterPinned,
            chrome: nil,
            rosterStores: 2161
        )
        XCTAssertEqual(rosterPinned.storeCount, 2161, "Command Center company tiles stay roster-pinned")
        XCTAssertEqual(
            PulseLaunch.metricPageHeroCard(rosterPinned, rows: rows).storeCount,
            4,
            "metric page hero unpins roster and counts PPH facts"
        )

        let prepThin = (1...8).map { n in
            MetricRow(
                section: .prepNotReady,
                division: "East",
                operationsOM: "OM",
                storeNumber: String(n),
                payload: n <= 5 ? ["pnr_rate_pct": 1.5] : [:]
            )
        }
        XCTAssertEqual(HeartbeatMath.metricStoreCount(.prepNotReady, rows: prepThin), 5)
        XCTAssertNotEqual(
            HeartbeatMath.metricStoreCount(.prepNotReady, rows: prepThin),
            8,
            "Soft FAIL forcing roster/all-row N on a thin metric"
        )
    ...
    func testArchitecture457DashboardNestsThisWeekDetail() {
        ...
        guard let command,
            ...
            return XCTFail("PhoneCommandCenterHome must stay the phone dashboard")
        }
        let home = String(command[start.lowerBound..<end.lowerBound])
        XCTAssertTrue(home.contains("PhoneDashboardMetricBlock("), home)
        XCTAssertFalse(home.contains("PhoneCompanyThisWeekBlock("), home)
        XCTAssertFalse(home.contains("PhoneSectionHeading"), "orphan THIS WEEK divider is gone")
        ...
        XCTAssertTrue(storeSrc?.contains("PhoneThisWeekChrome.chips(") == true)
        XCTAssertTrue(storeSrc?.contains("shouldShowDashboardThisWeekDetail()") == true)
        XCTAssertTrue(storeSrc?.contains("phoneScorecardChips(painted)") == true)
        ...
        guard let page,
              let blockStart = page.range(of: "struct PhoneCompanyThisWeekBlock"),
              let blockEnd = page.range(of: "struct PhoneSectionHeading")
        else {
            return XCTFail("PhoneCompanyThisWeekBlock is the shared This Week chrome")
        }
        let block = String(page[blockStart.lowerBound..<blockEnd.lowerBound])
    ...
    func testArchitecture458DashboardMergesResultAndPaintsFilterSeat() {
        ...
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.sales), "Sales Overview")
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.lostRevenue), "Loss Overview")
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.fiveStar), "5 Star Overview")
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.labor), "Labor Overview")
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.pph), "PPH Overview")
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.pickerScorecard), "Picker Overview")

        let companyCard = SectionSummary(
            section: .sales,
            storeCount: 2158,
            headline: 16_860_000,
            headlineLabel: "eComm sales",
        ...
        let jewelStores = [
            MetricRow(
                ...
                operationsOM: "",
                storeNumber: "3466",
                payload: ["sales_dollars": 1_000_000],
                textPayload: ["sales_grain": "store"]
            ),
            MetricRow(
                section: .sales,
                ...
                operationsOM: "",
                storeNumber: "3503",
                payload: ["sales_dollars": 1_006_799.79],
                textPayload: ["sales_grain": "store"]
            ),
        ]
        let companyGrain = MetricRow(
            section: .sales,
            division: "",
            operationsOM: "",
            storeNumber: "",
            payload: ["sales_dollars": 16_860_000],
            textPayload: ["sales_grain": "company"]
        )
        var jewel = DashboardFilters()
        jewel.division = "Jewel Osco"
        let jewelCard = PulseLaunch.dashboardSeatCard(
            companyCard,
            rows: jewelStores + [companyGrain],
            filters: jewel
        )
        XCTAssertEqual(jewelCard.headline ?? 0, 2_006_799.79, accuracy: 0.5)
        XCTAssertEqual(jewelCard.storeCount, 2)
        XCTAssertNotEqual(jewelCard.headline ?? 0, 16_860_000, accuracy: 0.5)

        let companyPaint = PulseLaunch.dashboardSeatCard(
            companyCard,
            rows: jewelStores,
            filters: DashboardFilters()
        )
        XCTAssertEqual(companyPaint.headline ?? 0, 16_860_000, accuracy: 0.5)
        XCTAssertEqual(companyPaint.storeCount, 2)

        let fiveStarCompany = SectionSummary(
            section: .fiveStar,
            storeCount: 2158,
            headline: 4.72,
        ...
        let fiveStarJewel = [
            ...
            MetricRow(
                ...
                operationsOM: "",
                storeNumber: "2",
                payload: ["star_rating": 4.7]
            ),
        ]
        let fiveStarPaint = PulseLaunch.dashboardSeatCard(
            fiveStarCompany,
            rows: fiveStarJewel,
            filters: jewel
        )
        XCTAssertEqual(fiveStarPaint.headline ?? 0, 4.8, accuracy: 0.01)
        XCTAssertEqual(fiveStarPaint.storeCount, 2)
        ...
        guard let command,
              let blockStart = command.range(of: "struct PhoneDashboardMetricBlock"),
              let blockEnd = command.range(of: "struct PhoneCommandHeroCard")
        else {
            return XCTFail("PhoneDashboardMetricBlock is the merged dashboard section")
        }
        let block = String(command[blockStart.lowerBound..<blockEnd.lowerBound])
        XCTAssertTrue(block.contains("PhoneCompactPageBanner("), block)
        XCTAssertTrue(block.contains("dashboardSectionOverviewTitle("), block)
        XCTAssertTrue(block.contains("cachedPhoneDashboardCard("), block)
        XCTAssertTrue(block.contains("cachedPhoneDashboardChips("), block)
        XCTAssertFalse(block.contains("PhoneCompanyThisWeekBlock("), block)
        XCTAssertFalse(block.contains("\"This Week\""), block)
        XCTAssertEqual(block.components(separatedBy: "PhoneScorecardRow(").count - 1, 1)
        ...
        XCTAssertTrue(storeSrc?.contains("dashboardSeatCard(") == true)
        XCTAssertTrue(storeSrc?.contains("shouldMergeDashboardResultAndThisWeek()") == true)
        XCTAssertTrue(storeSrc?.contains("PhoneThisWeekChrome.chips(") == true)
        XCTAssertTrue(storeSrc?.contains("phoneScorecardChips(painted)") == true)
        XCTAssertTrue(storeSrc?.contains("filters.summary") == true)
    }
    ...
    func testArchitecture459ThisWeekChromeIsMainActor() {
        ...
        guard let page,
              let start = page.range(of: "enum PhoneThisWeekChrome"),
              let end = page.range(of: "struct PhoneCompanyThisWeekBlock")
        else {
            return XCTFail("PhoneThisWeekChrome must stay the shared This Week helper")
        }
        XCTAssertTrue(
            page.contains("@MainActor\nenum PhoneThisWeekChrome"),
    ...
    func testArchitecture460PPHStaysOnPhoneDashboardUnderDynacap() {
        ...
        XCTAssertTrue(CommandCenterLayout.glanceSections.contains(.pph))
        XCTAssertTrue(CommandCenterLayout.phoneDashboardSections.contains(.pph))
        XCTAssertEqual(
            CommandCenterLayout.phoneDashboardSections,
            CommandCenterLayout.heroSections + CommandCenterLayout.glanceSections
        )
        let glance = CommandCenterLayout.glanceSections
        let dynacap = glance.firstIndex(of: .dynacap)
        let pph = glance.firstIndex(of: .pph)
        XCTAssertEqual(pph, dynacap.map { $0 + 1 }, "PPH must sit directly under Dynacap")
        XCTAssertEqual(CommandCenterLayout.dashboardSectionOverviewTitle(.pph), "PPH Overview")
        XCTAssertTrue(CommandCenterLayout.coversEveryDashboardSection())
        ...
        guard let command,
              let start = command.range(of: "struct PhoneCommandCenterHome"),
              let end = command.range(of: "struct PhoneDashboardMetricBlock")
        else {
            return XCTFail("PhoneCommandCenterHome must paint the phone Dashboard stack")
        }
        let home = String(command[start.lowerBound..<end.lowerBound])
        XCTAssertTrue(home.contains("phoneDashboardSections"), home)
        XCTAssertTrue(home.contains("shouldShowDashboardSection(section)"), home)
        XCTAssertTrue(home.contains("dashboard-\\(section.rawValue)"), home)
        XCTAssertFalse(home.contains("usablePaintedCards"), home)
        XCTAssertFalse(home.contains("dashboardCallouts"), home)
        ...
        XCTAssertTrue(
            dash?.contains("ForEach(Array(chips.enumerated()), id: \\.offset)") == true,
            "Soft FAIL ForEach(chips) id=label — Dynacap's PPH chip ate the PPH card"
        )
    }
    ...
    func testArchitecture461DynacapPPHChipUsesPPHSeat() {
        ...
        let dynacap = [
            MetricRow(
                ...
                operationsOM: "",
                storeNumber: "1",
                payload: ["dynacap_rate": 60.6, "utilization_pct": 20.25]
            )
        ]
        let pph = [
            MetricRow(
        ...
        XCTAssertEqual(HeartbeatMath.weekPurePPH(pph) ?? 0, 74.2, accuracy: 0.05)
        XCTAssertNil(HeartbeatMath.weekPurePPH(dynacap), "dynacap rows have no Pure PPH")
        XCTAssertEqual(
            HeartbeatMath.dynacapSeatPurePPH(dynacapRows: dynacap, pphRows: pph) ?? 0,
            74.2,
            accuracy: 0.05
        )
        XCTAssertNotEqual(
        ...
        let blank = HeartbeatMath.dashboardTableValues(.dynacap, rows: dynacap)
        XCTAssertEqual(blank.values.first, "60.6")
        XCTAssertEqual(blank.values.dropFirst().first, "—")

        let filled = HeartbeatMath.dashboardTableValues(.dynacap, rows: dynacap, pphRows: pph)
        XCTAssertEqual(HeartbeatMath.dashboardTableHeaders(.dynacap), ["Pcs/Hr", "PPH", "Util %"])
        XCTAssertEqual(filled.values[0], "60.6")
        XCTAssertEqual(filled.values[1], "74.2")
        XCTAssertNotEqual(filled.values[1], "60.6")
        XCTAssertEqual(filled.values[2], "20.25%")
        ...
        XCTAssertTrue(page?.contains("factRows(section: .pph") == true)
        XCTAssertTrue(page?.contains("pphRows: pphRows") == true)
    ...
    func testArchitecture462ParkHiddenDashboardAndCacheSeatPaint() {
        ...
        XCTAssertTrue(PulseLaunch.shouldLazyLoadPhoneDashboardSections())
        XCTAssertTrue(PulseLaunch.shouldKeepDashboardHostWarm())
        XCTAssertFalse(PulseLaunch.shouldRemountPageOnDestinationChange())
        XCTAssertTrue(PulseLaunch.shouldFillDynacapPPHFromPPHSeat())
        XCTAssertTrue(PulseLaunch.shouldPaintDashboardResultFromActiveSeat())
        ...
        guard let command,
            ...
            return XCTFail("PhoneCommandCenterHome must park when Pages leaves Dashboard")
        }
        let home = String(command[start.lowerBound..<end.lowerBound])
        XCTAssertTrue(home.contains("shouldParkHiddenPhoneDashboard()"), home)
        XCTAssertTrue(home.contains("isVisible"), home)
        XCTAssertTrue(home.contains("LazyVStack"), home)
        XCTAssertTrue(home.contains("phoneDashboardSections"), home)
        ...
        XCTAssertTrue(dash?.contains("isVisible: router.current == .dashboard") == true)
        ...
        XCTAssertTrue(storeSrc?.contains("phoneDashboardPaintKey") == true)
        XCTAssertTrue(storeSrc?.contains("filters.summary") == true)
        XCTAssertTrue(storeSrc?.contains("cachedPhoneDashboardRows(") == true)
        XCTAssertTrue(storeSrc?.contains("dashboardSeatCard(") == true)
    }
    ...
    func testArchitecture463PhoneDashboardHomeHasViewBuilderReturn() {
        ...
        let home = String(command[start.lowerBound..<end.lowerBound])
        XCTAssertTrue(home.contains("@ViewBuilder"), home)
        XCTAssertTrue(home.contains("Color.clear"), home)
        XCTAssertTrue(home.contains("visibleHome"), home)
        XCTAssertFalse(home.contains("let stack = ForEach"), home)
    }
    ...
    func testArchitecture465PagesNavPaintsBeforeRowWalks() {
        ...
        XCTAssertFalse(
            PulseLaunch.shouldShowParkedPhoneDashboard(isVisible: false, parked: false),
            "leaving Dashboard must not swap to Color.clear on the tap"
        )
        XCTAssertTrue(PulseLaunch.shouldShowParkedPhoneDashboard(isVisible: false, parked: true))
        XCTAssertFalse(PulseLaunch.shouldShowParkedPhoneSection(isVisible: true, parked: true))
        XCTAssertFalse(PulseLaunch.shouldShowParkedPhoneSection(isVisible: false, parked: false))
        XCTAssertTrue(PulseLaunch.shouldShowParkedPhoneSection(isVisible: false, parked: true))
        XCTAssertFalse(PulseLaunch.shouldWalkRowsForPhoneNavChrome(settledOnPage: false))
        XCTAssertTrue(PulseLaunch.shouldWalkRowsForPhoneNavChrome(settledOnPage: true))
        XCTAssertTrue(PulseLaunch.shouldParkHiddenPhoneDashboard())
        XCTAssertFalse(PulseLaunch.shouldRenderHiddenPhoneDashboardHeavy())
        XCTAssertTrue(PulseLaunch.shouldKeepDashboardHostWarm())
        XCTAssertFalse(PulseLaunch.shouldRemountPageOnDestinationChange())
        ...
        XCTAssertTrue(command?.contains("struct PhoneNavStableMount") == true)
        XCTAssertTrue(command?.contains("paintsPreparedCard") == true)
        let page = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SectionDetailView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(page?.contains("PhoneNavStableMount(token: heavyToken)") == true)
        XCTAssertTrue(page?.contains("usesMetricFactStoreCount: true") == true)
        XCTAssertTrue(page?.contains("allowRowWalk: true") == true)
        let hub = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/MainHubView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(hub?.contains("allowRowWalk: false") == true)
        let shared = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SharedViews.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(shared?.contains("allowRowWalk: false") == true)
    }
    ...
    func testArchitecture474PadPagesNavSkipsRowWalkAndPhoneStamp() {
        ...
        XCTAssertTrue(hub?.contains("struct HubBuildStamp") == true)
        XCTAssertTrue(hub?.contains("cachedNavHealth") == true)
        guard let hub,
              let sheetStart = hub.range(of: "struct CompactNavSheet")
        else {
            return XCTFail("CompactNavSheet missing")
        }
        let sheet = String(hub[sheetStart.lowerBound...])
        XCTAssertTrue(sheet.contains("HubBuildStamp()"), sheet)
        let page = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SectionDetailView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(page?.contains("cheapPhonePageChrome(section)") == true)
        XCTAssertTrue(page?.contains("if showTables, showStoreTable {") == true)
        let command = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/CommandCenterView.swift"),
            encoding: .utf8
        )
    ...
    func testArchitecture467LivePackDaysShoppersDensity() {
        ...
        XCTAssertEqual(CommandCenterLayout.heroBandHeight(phone: false, portrait: true, available: 1000), 128)
        XCTAssertEqual(HubLayout.MacReadable.glanceFloor, 196)
        XCTAssertEqual(CommandCenterLayout.sectionCardStackSpacing(phone: true, mac: false), 5)
        XCTAssertEqual(CommandCenterLayout.sectionCardStackSpacing(phone: false, mac: true), 16)
        XCTAssertEqual(HubLayout.phoneBannerVerticalPadding(), 3)
        ...
        let jewel = MetricRow(
            ...
            recordedOn: "202630",
            payload: [
                "sales_dollars": 14454.90,
                "sales_d0_dollars": 14454.90
            ],
            textPayload: [
                "sales_grain": "store",
                "sales_days": fullWeek
        ...
        let sunday = SalesRollupBuilder.dayRows(from: [jewel])
        XCTAssertEqual(sunday.map(\.label), ["Sunday"])
        XCTAssertEqual(sunday.first?.pack.sales ?? 0, 14454.90, accuracy: 0.01)
        XCTAssertNotEqual(sunday.first?.pack.sales ?? 0, 80184.84, accuracy: 1)
        XCTAssertEqual(SalesRollupBuilder.dayPacks(from: jewel).map(\.name), ["Sunday"])
        let companySunday = MetricRow(
            section: .sales,
            ...
            textPayload: ["sales_grain": "company", "sales_days": fullWeek]
        )
        XCTAssertEqual(
            SalesRollupBuilder.dayRows(from: [jewel], company: companySunday).map(\.label),
            ["Sunday"]
        )

        let labelsOnly = MetricRow(
            section: .sales,
            division: "Jewel Osco",
            operationsOM: "",
            storeNumber: "68",
            payload: ["sales_dollars": 14454.90],
            textPayload: ["sales_grain": "store", "sales_days": fullWeek]
        )
        XCTAssertTrue(SalesRollupBuilder.dayRows(from: [labelsOnly]).isEmpty)
        XCTAssertFalse(SalesRollupBuilder.rowHasSalesDayPayload(labelsOnly, index: 1))

        let twoDays = MetricRow(
            section: .sales,
            division: "Jewel Osco",
            operationsOM: "",
            storeNumber: "68",
            payload: [
                "sales_d0_dollars": 100,
                "sales_d0_orders": 2,
                "sales_d4_dollars": 40,
                "sales_d4_orders": 1,
                "sales_d1_dollars": 0
        ...
        XCTAssertEqual(SalesRollupBuilder.dayRows(from: [twoDays]).map(\.label), ["Sunday", "Thursday"])

        let pickerDays = MetricRow(
            section: .pickerScorecard,
            division: "Jewel Osco",
            operationsOM: "",
            storeNumber: "68",
            textPayload: [
                "days_json": #"[{"date":"2026-07-26","pph":"12.5","orders":"4"},{"date":"2026-07-27","pph":"N/A","orders":"","presub":"—"}]"#
            ]
        )
        XCTAssertEqual(PickerDaySnap.list(from: pickerDays).map(\.id), ["2026-07-26"])

        var store = DashboardFilters()
        store.store = "68"
        var ops = DashboardFilters()
        ops.om = "Ops"
        var district = DashboardFilters()
        district.district = "03"
        var division = DashboardFilters()
        ...
        XCTAssertTrue(PulseLaunch.shouldShowIndividualShoppers(filters: store))
        XCTAssertTrue(PulseLaunch.shouldShowIndividualShoppers(filters: ops))
        XCTAssertTrue(PulseLaunch.shouldShowIndividualShoppers(filters: district))
        XCTAssertTrue(PulseLaunch.shouldShowIndividualShoppers(filters: division))
        XCTAssertTrue(PulseLaunch.shouldShowPickerShoppersTable(filters: store))
        XCTAssertFalse(PulseLaunch.shouldShowIndividualShoppers(filters: company))
        XCTAssertFalse(PulseLaunch.shouldShowIndividualShoppers(filters: region))
        XCTAssertFalse(PulseLaunch.shouldLoadPickPathPickerOnPageOpen())
        XCTAssertTrue(PulseLaunch.shouldLoadPickPathPickerOnPageOpen(filters: store))
        XCTAssertTrue(PulseLaunch.shouldLoadPickPathPickerOnPageOpen(filters: ops))
        XCTAssertTrue(PulseLaunch.shouldLoadPickPathPickerOnPageOpen(filters: district))
        XCTAssertTrue(PulseLaunch.shouldLoadPickPathPickerOnPageOpen(filters: division))
        XCTAssertFalse(PulseLaunch.shouldLoadPickPathPickerOnPageOpen(filters: company))
        XCTAssertFalse(PulseLaunch.shouldLoadPickPathPickerOnPageOpen(filters: region))
        XCTAssertTrue(PulseLaunch.shouldPublishIndividualShoppers(dest: .pickerScorecard, filters: store))
        XCTAssertTrue(PulseLaunch.shouldPublishIndividualShoppers(dest: .pickPath, filters: division))
        XCTAssertFalse(PulseLaunch.shouldPublishIndividualShoppers(dest: .dashboard, filters: store))
        XCTAssertFalse(PulseLaunch.shouldPublishIndividualShoppers(dest: .pickerScorecard, filters: company))
        XCTAssertFalse(PulseLaunch.shouldPublishIndividualShoppers(dest: .sales, filters: region))
        ...
        XCTAssertTrue(sales?.contains("func rowHasSalesDayPayload") == true)
        XCTAssertTrue(sales?.contains("salesDayIndexes") == true)
        XCTAssertFalse(sales?.contains("sales_days\")?.split") == true)
        XCTAssertFalse(sales?.contains("components(separatedBy: \",\")") == true)
        let page = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SectionDetailView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(page?.contains("PhoneCompanyThisWeekBlock") == true)
        XCTAssertTrue(page?.contains("PhoneSectionHeading(title: \"This week\")") == true)
        XCTAssertTrue(page?.contains("PhoneSectionHeading(title: \"By Day\")") == true)
        XCTAssertTrue(page?.contains("FilteredShopperList(section: .pickPath)") == true)
        let dash = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/DashboardView.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(dash?.contains("Color.white") == true)
        XCTAssertTrue(dash?.contains(".headline") == true)
        let shared = try? String(
            contentsOf: root.appendingPathComponent("FulfillmentHeartbeat/Views/SharedViews.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(shared?.contains("PhoneCompactPageBanner") == true)
        XCTAssertTrue(shared?.contains("struct FilteredShopperList") == true)
    }
    ...
    func testArchitecture468LiveSqliteOverStaleFacts() throws {
        ...
        XCTAssertFalse(PulseLiveSource.shouldPublishFactsJSON())
        XCTAssertTrue(PulseLiveSource.shouldDeleteFactsJSONAfterSuccessfulCook())
        XCTAssertTrue(PulseLiveSource.shouldClearAndRedownloadStalePack(onDeviceWeek: "202628", packWeek: "202630"))
        XCTAssertFalse(PulseLiveSource.shouldClearAndRedownloadStalePack(onDeviceWeek: "202630", packWeek: "202630"))
        XCTAssertFalse(PulseLiveSource.shouldClearAndRedownloadStalePack(onDeviceWeek: "202630", packWeek: "202628"))
        XCTAssertFalse(PulseLiveSource.shouldClearAndRedownloadStalePack(onDeviceWeek: "", packWeek: "202630"))
        XCTAssertFalse(PulseLiveSource.shouldClearAndRedownloadStalePack(onDeviceWeek: "202628", packWeek: ""))
        XCTAssertTrue(PulseLiveSource.shouldIgnoreOlderFactsStamp(factsStamp: "HB-0828.419", packStamp: "HB-0828.474"))
        XCTAssertFalse(PulseLiveSource.shouldIgnoreOlderFactsStamp(factsStamp: "HB-0828.474", packStamp: "HB-0828.419"))

        let fullWeek = "Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday"
        let sqlite = MetricRow(
            section: .sales,
            division: "Jewel Osco",
            operationsOM: "",
            storeNumber: "68",
        ...
        let facts = MetricRow(
            ...
            recordedOn: "202628",
            payload: [
                "sales_dollars": 80184.84,
                "sales_d1_dollars": 16036.97,
                "sales_d2_dollars": 16036.97,
                "sales_d3_dollars": 16036.97,
                "sales_d4_dollars": 16036.97,
                "sales_d5_dollars": 16036.96
            ],
            textPayload: [
                "sales_grain": "store",
                "sales_week": "202628",
        ...
        let staleDays = SalesRollupBuilder.dayRows(from: [facts])
        XCTAssertEqual(
            staleDays.map(\.label),
            ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
        )
        XCTAssertEqual(
            staleDays.reduce(0) { $0 + ($1.pack.sales ?? 0) },
            80184.84,
            accuracy: 0.01
        )

        let live = PulseLiveSource.liveSalesRows(sqlite: [sqlite], facts: [facts], sqliteUsable: true)
        let days = SalesRollupBuilder.dayRows(from: live)
        XCTAssertEqual(days.map(\.label), ["Sunday"])
        XCTAssertEqual(days.first?.pack.sales ?? 0, 14454.90, accuracy: 0.01)
        XCTAssertEqual(HeartbeatMath.salesHeadlineDollars(live[0]), 14454.90, accuracy: 0.01)
        XCTAssertNotEqual(HeartbeatMath.salesHeadlineDollars(live[0]), 80184.84, accuracy: 1)
        XCTAssertEqual(SalesPack(rows: live).sales ?? 0, 14454.90, accuracy: 0.01)
        XCTAssertNotEqual(SalesPack(rows: live).sales ?? 0, 80184.84, accuracy: 1)

        let missing = PulseLiveSource.liveSalesRows(sqlite: [], facts: [facts], sqliteUsable: false)
        XCTAssertTrue(missing.isEmpty)
        XCTAssertTrue(SalesRollupBuilder.dayRows(from: missing).isEmpty)
        XCTAssertNotEqual(HeartbeatMath.salesHeadlineDollars(facts), 14454.90, accuracy: 1)

        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("hb-468-\(UUID().uuidString).sqlite")
        defer { try? FileManager.default.removeItem(at: tmp) }
        try PulseSQLite.write(rows: [sqlite], uploads: [], seeded: true, to: tmp)
        XCTAssertEqual(PulseSQLite.salesWeek(at: tmp), "202630")
        XCTAssertTrue(
            PulseLiveSource.shouldClearAndRedownloadStalePack(
                onDeviceWeek: "202628",
                packWeek: PulseSQLite.salesWeek(at: tmp)
            )
        )
        ...
        let script = try String(
            contentsOf: root.appendingPathComponent("Tools/HeartbeatIngest/publish-cloud.sh"),
            encoding: .utf8
        )
        XCTAssertTrue(script.contains("delete_object \"facts.json\""))
        XCTAssertTrue(script.contains("COOK FAILED: facts.json is still in the bucket"))
