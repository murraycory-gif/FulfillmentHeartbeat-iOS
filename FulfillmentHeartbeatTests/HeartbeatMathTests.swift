...
final class HeartbeatMathTests: XCTestCase {
    ...
    /// 467: Store / Ops / District / Division filters list real shoppers and bind facts.
    func testFilterShoppersOnStoreOpsDistrictDivision() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.467")
        XCTAssertEqual(
            FilterFocus.allCases.map(\.rawValue),
            ["region", "division", "district", "om", "store"]
        ...
        func picker(
            ...
            MetricRow(
                ...
                operationsOM: om,
                storeNumber: store,
                payload: ["pph": pph, "orders": 20],
                textPayload: ["shopper_id": id, "shopper_name": id, "district": district]
            )
        }

        let rows = [
            picker("ONE68", store: "68", division: "Jewel Osco", district: "J9", om: "Ops A", pph: 90),
            picker("TWO68", store: "68", division: "Jewel Osco", district: "J9", om: "Ops A", pph: 40),
            MetricRow(
                section: .pickPathPicker,
                division: "Jewel Osco",
                operationsOM: "Ops A",
                storeNumber: "68",
                payload: ["compliance_pct": 81.36],
                textPayload: ["shopper_id": "ONE68", "shopper_name": "ONE68", "district": "J9"]
            ),
            picker("J9B", store: "70", division: "Jewel Osco", district: "J9", om: "Ops B", pph: 55),
            picker("OTHER", store: "10", division: "Jewel Osco", district: "J2", om: "Ops A", pph: 60),
            picker("SHAW", store: "200", division: "Shaws", district: "S1", om: "Ops C", pph: 70),
            MetricRow(
                section: .sales,
                division: "Jewel Osco",
                operationsOM: "Ops A",
        ...
        var storeSeat = DashboardFilters()
        storeSeat.store = "68"
        XCTAssertTrue(FilterFocus.sheetChips(filters: storeSeat).contains(.shopper))
        XCTAssertEqual(
            PulseLaunch.filterShopperChoices(rows: rows, filters: storeSeat).map(\.label),
            ["ONE68", "TWO68"]
        )
        ...
        ops.om = "Ops A"
        XCTAssertEqual(
            Set(PulseLaunch.filterShopperChoices(rows: rows, filters: ops).map(\.label)),
            Set(["ONE68 · 68", "TWO68 · 68", "OTHER · 10"])
        )

        var district = DashboardFilters()
        district.district = "J9"
        XCTAssertEqual(
            Set(PulseLaunch.filterShopperChoices(rows: rows, filters: district).map(\.label)),
            Set(["ONE68 · 68", "TWO68 · 68", "J9B · 70"])
        )

        var division = DashboardFilters()
        division.division = "Jewel Osco"
        let divisionLabels = PulseLaunch.filterShopperChoices(rows: rows, filters: division).map(\.label)
        XCTAssertEqual(divisionLabels.count, 4)
        XCTAssertFalse(divisionLabels.contains { $0.contains("SHAW") })

        XCTAssertTrue(PulseLaunch.filterShopperChoices(rows: rows, filters: DashboardFilters()).isEmpty)
        var region = DashboardFilters()
        region.region = "East Region"
        XCTAssertTrue(PulseLaunch.filterShopperChoices(rows: rows, filters: region).isEmpty)
        XCTAssertFalse(FilterFocus.sheetChips(filters: region).contains(.shopper))
        XCTAssertFalse(FilterFocus.sheetChips(filters: DashboardFilters()).contains(.shopper))
        ...
        XCTAssertEqual(
            PulseLaunch.filterShopperChoices(rows: rows + [ghost], filters: storeSeat).count,
            2
        )

        storeSeat.shopper = PulseLaunch.shopperChoiceID(rows[0])
        let bound = HeartbeatMath.filtered(rows, filters: storeSeat)
        let pickers = bound.filter { $0.section == .pickerScorecard }
        XCTAssertEqual(pickers.compactMap(\.shopperId), ["ONE68"])
        XCTAssertEqual(pickers.first?.number("pph"), 90)
        let path = bound.filter { $0.section == .pickPathPicker }
        XCTAssertEqual(path.compactMap(\.shopperId), ["ONE68"])
        XCTAssertEqual(bound.filter { $0.section == .sales }.count, 1)
        XCTAssertEqual(
            CommandCenterLayout.overviewBannerCopy(filters: storeSeat, weekWindow: nil),
            "ONE68 Shopper | Current Week"
        )
        var divisionOnly = DashboardFilters()
        divisionOnly.division = "Jewel Osco"
        XCTAssertEqual(
            CommandCenterLayout.overviewBannerCopy(filters: divisionOnly, weekWindow: nil),
