...
final class HeartbeatMathTests: XCTestCase {
    ...
    func testArchitecture427MacCatalystHasAppCategory() throws {
        ...
        if FileManager.default.fileExists(atPath: plistURL.path) {
            ...
            XCTAssertTrue(pbx.contains("CURRENT_PROJECT_VERSION = 770;"))
        }
    ...
    func testArchitecture440SeatFirstShellPhonePadOnly() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.444")
        XCTAssertFalse(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertFalse(PulseLaunch.shouldMountSeatHub(mac: false, phone: true, pad: false))
        ...
        XCTAssertFalse(PulseLaunch.shouldHideShopperListsOnSeatShell(mac: false, filters: company))
        ...
        XCTAssertTrue(pbx?.contains("CURRENT_PROJECT_VERSION = 770;") == true)
    }
    ...
    func testArchitecture441FirstPrinciplesSeatShell() {
        ...
        XCTAssertFalse(
            ...
            "Scoreboard must not stack section pages"
        )
        XCTAssertFalse(PulseLaunch.shouldOpenSectionFromSeatScoreboard())
    }

    /// Architecture P1: grain-first is its own tip, flag default OFF.
    func testArchitecture442SeatFirstPhoneFlagDefaultOff() {
        XCTAssertEqual(BuildStamp.id, "HB-0828.444")
        XCTAssertFalse(PulseLaunch.seatFirstPhoneEnabled())
        XCTAssertFalse(PulseLaunch.shouldEnableSeatFirstOnIPad())
        XCTAssertTrue(PulseLaunch.shouldKeepSectionCompactNavWhileSeatFlagOff())
        ...
        XCTAssertFalse(PulseLaunch.shouldMountSeatHub(mac: false, phone: true, pad: false))
        XCTAssertFalse(PulseLaunch.shouldMountSeatHub(mac: false, phone: false, pad: true))
        ...
        XCTAssertTrue(launch?.contains("static func seatFirstPhoneEnabled() -> Bool { false }") == true)
        XCTAssertFalse(
            launch?.contains("static func seatFirstPhoneEnabled() -> Bool { true }") == true
        )
    }

    /// Cory design bar: KPI tiles + instant seat switcher. Flag stays OFF.
    func testArchitecture443SeatDesignBar() {
        ...
        XCTAssertFalse(shell?.contains("PhoneSectionPage(") == true)
        XCTAssertFalse(
    ...
    /// App Design memo v3: one-glance KPIs + actionable strip + Clear/crumb/drill.
    func testArchitecture444AppDesignMemoV3SeatShell() {
        ...
        XCTAssertTrue(shell?.contains("childTables") == true)
        XCTAssertTrue(shell?.contains("shouldShowPickersOnSeatPage") == true)
