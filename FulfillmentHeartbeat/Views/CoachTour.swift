...
enum CoachTour: Equatable {
    ...
    private static let welcomeSteps: [CoachStep] = [
        ...
        CoachStep(
            ...
            body: "Dashboard is the heartbeat. Swipe between scorecards (Loss Revenue → 5 Star → Pick Path → Prep → Dynacap → Schedule → Picker → PPH → Labor → Checklist). Filters in the header apply everywhere. Share sends the recap. Upload lives in Settings — it is not in the swipe path."
        ),
        CoachStep(
            icon: "hand.tap.fill",
        ...
        case .upload:
            return [
                CoachStep(
                    icon: "doc.badge.plus",
                    heading: "Master load",
                    body: "Pick one .xlsx that has a tab per KPI. Name the sheets Lost Revenue, 5 Star, Pick Path, Path Picker, Prep, Dynacap, Schedule, PPH, Labor, and Picker ScoreCard — or leave the Power BI headers and we map them. Link the file so next week you can reload from the same place."
                ),
                CoachStep(
